...
| Files and paths | The [native namespace service](../src/userspace/moss_namespace_service.c) handles a flat root; the [file service](../src/userspace/moss_file_service.c) keeps volatile file contents and returns file capabilities. Opened file capabilities support size queries and atomic append within one service request. The [process service](../src/userspace/moss_process_service.c) now holds an internal file descriptor view with shared open-description offsets; a raw IPC probe exercises `dup`, `fork` and `exec`. The [mlibc open/read/write path](../third_party/mlibc/sysdeps/moss/sysdeps.cpp) still calls the kernel, whose [open handler](../src/kernel/src/syscall_table.cpp) invokes kernel VFS. | Unify native file descriptors with kernel-backed console and pipe descriptors, add per-descriptor close-on-exec, current directories, traversal and remaining metadata, then redirect mlibc. Migrate selected BusyBox workflows before deleting the kernel VFS path. Define filesystem-service restart and file-object lifetime separately from namespace restart. |
| Executable images and memory | The kernel now accepts explicit pages and an initial stack through a factory-authorized native domain spawn. Its [exec path](../src/kernel/src/syscall_table.cpp) still parses and maps ELF. Anonymous executable `mmap` is denied and `mprotect` returns `ENOSYS`; this is a fail-closed interim rule, not the immutable-version authority in [ADR-0026](adr/0026-require-explicit-authority-for-executable-memory.md). | Introduce the userspace loader and pager contracts, then enforce immutable code versions and approval-instance rights across aliases, repaging, revocation and service failure before moving ordinary loading out of the kernel. Verified bootstrap authority is a separate [ADR-0027](adr/0027-establish-initial-authority-through-verified-boot.md) gate. |
...
2. Establish one POSIX descriptor view for regular files, pipes and console
   ...
   process service and clones references for managed children. The production
   probe uses its raw IPC interface to verify `dup`, `fork` and `exec`; it does
   not route libc file calls or console and pipe descriptors through that view.
