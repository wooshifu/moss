...
| Files and paths | The [native namespace service](../src/userspace/moss_namespace_service.c) handles a flat root; the [file service](../src/userspace/moss_file_service.c) keeps volatile file contents and returns file capabilities. Opened file capabilities support size queries and atomic append within one service request. The [process service](../src/userspace/moss_process_service.c) holds an internal descriptor view with shared open-description offsets, duplication, status queries and mutable close-on-exec. It can import a caller's transferable File Object Capability without repeating namespace lookup; production verifies that a default send-only handle cannot be imported. The [mlibc open/read/write path](../third_party/mlibc/sysdeps/moss/sysdeps.cpp) still calls the kernel, whose [open handler](../src/kernel/src/syscall_table.cpp) invokes kernel VFS. | Unify native file descriptors with kernel-backed console and pipe descriptors, current directories, traversal and remaining metadata, then redirect mlibc. Migrate selected BusyBox workflows before deleting the kernel VFS path. Define filesystem-service restart and file-object lifetime separately from namespace restart. |
...
2. Establish one POSIX descriptor view for regular files, pipes and console
   ...
   view; a send-only lookup remains nondelegable. It does not route libc file
   calls or console and pipe descriptors through that view.
