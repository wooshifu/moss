...
The file service receives the raw boot archive as a read-only Memory Object, parses its root entries in userspace and serves boot images without charging them to the volatile-file budget. The supervisor copies its Loader probes from that service into private files, seals them and checks that mutation is rejected. The Loader also requires a successful seal before reading any incoming image; a named public file fails that gate. Ordinary `exec` and BusyBox file descriptors still use kernel VFS, so this remains a boot-file data boundary rather than completion of the filesystem and Loader ADRs.
...
1. Close process-service recovery first. Fault injection showed that an old
   ...
   cannot call the replacement endpoint. The production recovery probe now
   sends an unscoped caller's `FD_WAIT` through the real process service,
   confirms that the console service holds its Reply, kills the process
   service, and observes the caller still blocked. It then stops the reply
   holder and requires `EPIPE` from the pending call. The management shell
   still uses kernel-backed stdio, so the console waiter's root-only count
   isolates this probe. Exercise orphaned exit records under concurrent
   service loss before treating process recovery as complete.
