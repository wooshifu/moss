...
| Files and paths | The [native namespace service](../src/userspace/moss_namespace_service.c) handles a flat root; the [file service](../src/userspace/moss_file_service.c) keeps volatile file contents and returns file capabilities. Opened file capabilities support size queries and atomic append within one service request. The [process service](../src/userspace/moss_process_service.c) now holds an internal file descriptor view with shared open-description offsets, minimum and exact-target duplication, open-status queries and mutable per-descriptor close-on-exec; a production probe exercises these operations across `fork` and `exec`. The [mlibc open/read/write path](../third_party/mlibc/sysdeps/moss/sysdeps.cpp) still calls the kernel, whose [open handler](../src/kernel/src/syscall_table.cpp) invokes kernel VFS. | Unify native file descriptors with kernel-backed console and pipe descriptors, current directories, traversal and remaining metadata, then redirect mlibc. Migrate selected BusyBox workflows before deleting the kernel VFS path. Define filesystem-service restart and file-object lifetime separately from namespace restart. |
