...
The namespace service resolves flat root names and delegates per-file SEND authority; an unknown path fails lookup. The file service currently holds at most 16 volatile files within a 64 KiB content budget. Positional reads and writes transfer a one-page Memory Object with attenuated mapping rights, although Memory Objects can span multiple pages. Restarting the namespace service preserves those files, while restarting the file service loses their contents. The supervisor restarts dependent services and shell with new endpoint capabilities when either service dies. The kernel VFS still loads images and handles BusyBox file operations. A general namespace, larger service transfers and POSIX file compatibility remain to be migrated.
