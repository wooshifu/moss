...
The production initramfs contains `/init.elf`, `/code-authority-service.elf`, `/file-service.elf`, `/namespace-service.elf`, `/process-service.elf`, `/pipe-service.elf`, `/console-service.elf`, `/moss-file.elf`, `/moss-process.elf`, `/moss-domain.elf` and `/busybox.elf`. The embedded init trampoline starts `/init.elf`, which launches the capability-backed services and supervises interactive ash with `PATH=/`. `/moss-file.elf` opens `/scratch` through the namespace service and reads or writes through the returned file endpoint, independently of the kernel VFS. The Code Authority Service receives only delegated approval rights and requests from the supervisor; executable policy for ordinary programs remains pending. The validation initramfs instead provides `/validation.elf`, including the signal regression cases, which the same trampoline selects first. Standalone shell applet lookup re-executes `/busybox.elf`, so selected commands and `sh` work without applet symlinks or `/proc/self/exe`. Job control and a terminal line editor remain disabled. RV64 uses `rv64imac/lp64`; builtins supply compiler-generated arithmetic helpers, including the soft-float helpers needed by mlibc. There is no dynamic linker or Linux binary-compatibility promise.

The namespace service resolves flat root names and delegates per-file SEND authority; an unknown path fails lookup. The initial supervisor receives a read-only Memory Object containing the raw boot archive and passes reduced authority to the file service. That service parses the archive and publishes its regular root files as read-only File Objects, rejecting writable path opens before returning authority, while retaining a separate limit of 16 volatile files and 64 KiB of mutable content. The Loader probe image is seeded from a boot File Object instead of the kernel VFS. Positional reads and writes still transfer one-page Memory Objects with attenuated mapping rights, although Memory Objects can span multiple pages. Restarting the namespace service preserves file objects; restarting the file service reloads boot files and loses volatile contents. The supervisor restarts dependent services and shell with new endpoint capabilities when either service dies. Kernel VFS still loads ordinary executables and handles BusyBox file operations. A general namespace, larger service transfers and POSIX file compatibility remain to be migrated.
