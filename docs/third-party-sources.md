...
The namespace service recognizes only `/scratch` and delegates only file SEND authority; an unknown path fails lookup. The file service currently holds one volatile file of at most 255 bytes. Restarting the namespace service preserves that file, while restarting the file service loses its contents. The supervisor restarts the dependent services and shell with new endpoint capabilities when either service dies. The kernel VFS still loads images and handles BusyBox file operations. A general namespace, larger shared-memory data transfers and POSIX file compatibility remain to be migrated.
