...
The production initramfs contains only `/busybox.elf`; the embedded init trampoline starts interactive ash directly with `PATH=/`. The validation initramfs instead provides `/validation.elf`, including the signal regression cases, which the same trampoline selects before BusyBox. Standalone shell applet lookup re-executes `/busybox.elf`, so selected commands and `sh` work without applet symlinks or `/proc/self/exe`. Job control and a terminal line editor remain disabled. RV64 uses `rv64imac/lp64`; builtins supply compiler-generated arithmetic helpers, including the soft-float helpers needed by mlibc. There is no dynamic linker or Linux binary-compatibility promise.
