...
Production boot starts a userspace process service and runs `/moss-process.elf probe` through the real shell. The probe transfers observation authority for two native child domains, closes its own domain handles, reads their exit status through service-issued badged session capabilities, and releases each registration. Numeric compatibility IDs do not authorize a lookup. The boot test also kills and restarts the process service while retaining the independent file and namespace services; registrations from the crashed service incarnation are lost. Ordinary libc `fork`, `execve` and `waitpid` still use the kernel compatibility implementation; this registry is the first process-service path, not the completed POSIX migration.
