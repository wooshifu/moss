...
Production boot starts a userspace process service, registers init as compatibility ID 1, and starts the shell with a badged child session in its auxiliary vector. Managed mlibc `getpid`, `getppid`, `fork`, `execve`, and `waitpid` use that session; processes without a startup session retain the kernel compatibility path. `SYS_EXECVE_CAP` retains the selected session handle and publishes its number in `MOSS_AT_STARTUP_CAP` without adding an environment variable. The boot probe exercises the managed libc fork/exec/wait route with an empty environment, then transfers observation authority for separate native child domains, checks service-issued badged sessions, reservation cancellation, child identity, release denial, readiness, parent-authorized wait-by-ID and wait-any reaping. The service compares domain capabilities directly to prevent duplicate registration even if diagnostic PIDs are reused. Numeric compatibility IDs do not authorize a lookup or wait. The boot test kills and restarts the process service while retaining the independent file and namespace services; registrations from the crashed service incarnation are lost. Compatibility signal delivery, process groups, asynchronous child notification, and service-state reconstruction remain outside this slice.
