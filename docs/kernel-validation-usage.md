...
`users.ipc` and `moss-production-boot` exercise capability-backed control IPC and Memory Objects used by production. The production file-descriptor probe opens a file with close-on-exec, duplicates and replaces descriptors, queries open status, checks minimum-descriptor allocation and descriptor flags, distinguishes missing paths and bad descriptors, and imports a transferable File Object Capability while rejecting a send-only one. It lists the native flat root, checks directory metadata and a shared directory cursor across duplication, then forks and executes a new managed image: a constructor sees the original closed, the new image retains the replacement, and the parent retains its original descriptor. Finally, it fills its descriptor table and verifies that a rejected `O_TRUNC` open leaves the file contents intact.
