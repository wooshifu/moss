...
`users.ipc/domain_control` checks that `SYS_FORK_DOMAIN` returns a child capability only to the creator, has no POSIX parent/wait status, keeps inspection, observation and termination rights separate, and retains a native exit result through `SYS_DOMAIN_STATUS` after the diagnostic PID is retired. Production `moss-init` uses domain capabilities to terminate and wait for services and shells; `waitpid` reaps only orphaned POSIX descendants.
