...
`users.ipc` and `moss-production-boot` exercise capability-backed control IPC and Memory Objects used by production. The production file-descriptor probe opens a file with close-on-exec, duplicates it, forks, and executes a new managed image: a constructor sees the original closed, the new image retains the duplicate, and the parent retains its original descriptor.
