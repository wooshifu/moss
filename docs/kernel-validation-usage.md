...
Production boot starts a userspace process service and runs `/moss-process.elf probe` through the real shell. The probe transfers observation authority for native child domains, closes its own domain handles, reads exit status through service-issued badged session capabilities, and releases those registrations. The service compares domain capabilities directly to prevent duplicate registration even if diagnostic PIDs are reused. The probe registers its own domain as a parent, preallocates child identities and inheritable badged sessions before native fork, then attaches each child domain through the parent's session. It checks reservation cancellation, child identity, release denial, child readiness, parent-authorized wait-by-ID and wait-any reaping. Numeric compatibility IDs do not authorize a lookup or wait. The boot test kills and restarts the process service while retaining the independent file and namespace services; registrations from the crashed service incarnation are lost. Ordinary libc `fork`, `execve` and `waitpid` still use the kernel compatibility implementation; this registry is an incremental process-service path, not the completed POSIX migration.
