...
`users.ipc` and `moss-production-boot` exercise capability-backed control IPC and Memory Objects used by production. The production file-descriptor probe opens a file with close-on-exec, duplicates and replaces descriptors, queries open status, checks minimum-descriptor allocation and descriptor flags, and imports a transferable File Object Capability while rejecting a send-only one. It then forks and executes a new managed image: a constructor sees the original closed, the new image retains the replacement, and the parent retains its original descriptor.
