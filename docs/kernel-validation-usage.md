...
`users.ipc/domain_control` checks that `SYS_FORK_DOMAIN` returns a child capability only to the parent, that inspection, observation and termination rights remain separate, and that a held domain capability still reports exit after POSIX reaping. Production `moss-init` uses child domain capabilities to terminate and wait for services and shells; `waitpid` still provides compatibility reaping.
...
`users.ipc/domain_wait_any` checks waiting for the first exit in a capability list, observation after POSIX reaping, duplicate target rejection and observe-right enforcement. Production `moss-init` uses this list to identify exited services and shells; `waitpid` remains for compatibility zombie reaping, including orphaned shell descendants.
