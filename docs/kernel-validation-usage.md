...
`users.ipc` and `moss-production-boot` exercise capability-backed control IPC and Memory Objects used by production. The production file-descriptor probe opens a file with close-on-exec, duplicates and replaces descriptors, queries open status, checks minimum-descriptor allocation and descriptor flags, distinguishes missing paths and bad descriptors, and imports a transferable File Object Capability while rejecting a send-only one. It compares descriptor metadata with a namespace path query, checks a missing and an invalid path, and lists the native flat root with a shared directory cursor across duplication. It then forks and executes a new managed image: a constructor sees the original closed, the new image retains the replacement, and the parent retains its original descriptor. Finally, it fills its descriptor table and verifies that a rejected `O_TRUNC` open leaves the file contents intact.
`users.ipc/memory_object` maps a three-page object with writable and read-only aliases, checks page boundaries and zero fill, then verifies shared writes across fork and capability transfer. It also rejects unaligned sizes and checks that closing handles preserves mappings until whole-region unmap.
The same validation denies raw boot-archive access outside the initial supervisor. Production boot passes its read-only archive capability to the file service, seeds the Loader probe through that service, and reads `/busybox.elf` through the managed descriptor path beyond the 64 KiB mutable-file budget. It rejects writable opens and truncation of that boot file before allocating a descriptor.
The native descriptor probe also seeks near the volatile file-service content limit, rejects a one-byte write with `NO_SPACE`, and checks that the existing file bytes and shared offset remain usable.
...
`users.ipc/domain_control` checks that `SYS_DOMAIN_SELF` returns a capability for the caller, `SYS_FORK_DOMAIN` returns a child capability only to the creator, native children have no POSIX parent/wait status, unrelated PID-based signals are rejected, and even an ordinary fork child cannot change its native parent's CPU affinity by PID. Inspection, observation, signaling and forced termination use distinct capability bits; signaling includes SIGKILL, while `DOMAIN_TERMINATE` does not authorize other signals. A signal-only domain capability can probe and signal that exact running domain without using its diagnostic PID, but cannot inspect, observe or call the separate termination operation. `SYS_DOMAIN_SAME` compares the domain incarnation behind two inspection capabilities, including distinct self handles and an exited child, without relying on diagnostic PID values; wrong type, missing rights and closed handles are rejected. `SYS_DOMAIN_STATUS` retains a native exit result after the diagnostic PID is retired. Production `moss-init` uses domain capabilities to terminate and wait for services and shells; `waitpid` reaps only orphaned POSIX descendants. Production recovery checks use termination capabilities explicitly delegated to the management shell.
`users.ipc/domain_selection` checks that the empty native fork omits even parent handles opted into inheritance, while `SYS_FORK_DOMAIN_INHERIT` creates a native child with only those opted-in handles, preserves them through exec, and gives it no POSIX parent or wait status. Explicit native selection copies only listed handles at stable numbers with reduced rights and rejects duplicate or unauthorized selections. A selected handle survives a real exec; only handles marked for later fork inheritance reach ordinary fork children. It also checks that ordinary fork inheritance and exec retention can be changed independently, including rejecting retention without `DUPLICATE` authority. A scoped child delegates factory authority to spawn another native domain; the scope must count and terminate the child, its forked child and the spawned domain. Production `moss-init` selects each service's and shell's handles and rights.
...
Production boot starts a userspace process service, registers init as compatibility ID 1, and starts the shell with a badged child session in its auxiliary vector. Managed mlibc `getpid`, `getppid`, `fork`, `execve`, `waitpid`, `getpgid`, `getsid`, `setpgid`, `setsid` and `kill` use that session; processes without a startup session retain the kernel compatibility path. `SYS_EXECVE_CAP` retains the selected session handle and publishes its number in `MOSS_AT_STARTUP_CAP` without adding an environment variable. The boot probe exercises the managed libc fork/exec/wait route with an empty environment, then transfers observation and signal authority for separate native child domains, checks service-issued badged sessions, reservation cancellation, child identity, release denial, readiness, parent-authorized wait-by-ID and wait-any reaping. It verifies that a child can attach its own domain and the parent can repeat that attachment, then probes that a managed process can signal itself or its child but not its parent. The probe also covers inherited process groups and sessions, parent-controlled group changes, group-directed signal and wait, and session creation. It creates more orphan generations than the process service has record slots: each child must observe init as its new parent, and exited adopted children are reclaimed on the next service request. A reserved child can attach itself after its parent exits; an unattached reservation expires after five seconds. The service compares domain capabilities directly to prevent duplicate registration even if diagnostic PIDs are reused. Numeric compatibility IDs do not authorize a lookup, wait or signal without the matching badged session. Group signals currently reach only the caller and its direct children; broadcast `kill(-1)`, terminal foreground groups, compatibility credentials, asynchronous child notification, and service-state reconstruction remain outside this slice. The boot test kills and restarts the process service while retaining the independent file and namespace services; registrations from the crashed service incarnation are lost.

The process-service restart probe also holds a real `FD_WAIT` call across receiver death: init checks that the console service has accepted the transferred Reply, observes the unscoped caller still blocked after the process service exits, then requires `EPIPE` when the console service holding that Reply exits. The shell itself still uses kernel stdio; the root-only console waiter count is empty before this native probe starts.
