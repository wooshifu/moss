...
static void print_memory(struct TopInfo *info) {
    ...
    if (page_kb == 0) page_kb = 4;  // default 4KB pages
    ...
    if (usage_pct > 80) print(ESC_RED);
    else if (usage_pct > 50) print(ESC_YELLOW);
    else print(ESC_GREEN);
    print_ulong(used_kb);
    print("KB used");
    print(ESC_RESET "  ");
...
static void print_cpus(struct TopInfo *info,
                       struct TopInfo *prev) {
    unsigned long nr_cpus = info->nr_cpus;
    if (nr_cpus > TOP_MAX_CPUS) nr_cpus = TOP_MAX_CPUS;
    ...
    for (unsigned long i = 0; i < nr_cpus; i++) {
        ...
        if (load > 70) print(ESC_RED);
        else if (load > 40) print(ESC_YELLOW);
        else print(ESC_GREEN);
        ...
        if (prev != 0 && prev->uptime_ns > 0) {
            ...
            if (dt > 0) {
                unsigned long idle_delta =
                    info->cpu_idle_time_ns[i] - prev->cpu_idle_time_ns[i];
...
static void print_process_table(struct TopInfo *info,
                                struct TopInfo *prev) {
    // Table header
    print(ESC_BOLD);
    ...
    for (unsigned long i = 0; i < info->nr_processes; i++) {
        ...
        if (st == 2) print(ESC_GREEN);      // Running
        else if (st == 3) print(ESC_YELLOW); // Blocked
        else if (st >= 4) print(ESC_RED);    // Dead/Zombie
        print(state_name(st));
        print(ESC_RESET);
        print("  ");
