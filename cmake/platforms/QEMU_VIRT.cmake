...
# 设置链接器脚本路径 (向后兼容)
set(LINKER_SCRIPT_TEMPLATE "${CMAKE_SOURCE_DIR}/platform/qemu-virt/${MOSS_TARGET_ARCH}/linker.ld.in")
...
if(EXISTS ${LINKER_SCRIPT_TEMPLATE})
    ...
    set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -T ${LINKER_SCRIPT_OUTPUT}" PARENT_SCOPE)
    ...
    if(EXISTS ${FALLBACK_LINKER_SCRIPT})
        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -T ${FALLBACK_LINKER_SCRIPT}" PARENT_SCOPE)
...
# 生成 QEMU 运行脚本 (向后兼容)
set(QEMU_SCRIPT_TEMPLATE "${CMAKE_SOURCE_DIR}/platform/qemu-virt/run_qemu.sh.in")
set(QEMU_SCRIPT_OUTPUT "${CMAKE_BINARY_DIR}/run_qemu.sh")

if(EXISTS ${QEMU_SCRIPT_TEMPLATE})
    configure_file(
        ${QEMU_SCRIPT_TEMPLATE}
        ${QEMU_SCRIPT_OUTPUT}
        @ONLY
    )
    file(CHMOD ${QEMU_SCRIPT_OUTPUT} PERMISSIONS OWNER_READ OWNER_WRITE OWNER_EXECUTE GROUP_READ GROUP_EXECUTE WORLD_READ WORLD_EXECUTE)
