...
function(moss_ensure_uv)
    ...
    find_program(
        UV_EXECUTABLE uv HINTS "$ENV{HOME}/.cargo/bin" "$ENV{HOME}/.local/bin" "$ENV{USERPROFILE}/.cargo/bin"
...
function(moss_check_prerequisites arch)
    ...
    execute_process(
        COMMAND ${UV_EXECUTABLE} run "${_script}" --json --arch ${arch} --build-dir "${CMAKE_BINARY_DIR}"
    ...
    if(NOT _rc EQUAL 0)
        message(FATAL_ERROR "Prerequisite check failed (exit ${_rc}).\n" "${_stderr}\n${_json_output}")
    endif()
    ...
    if(NOT _status STREQUAL "ok")
        ...
        message(FATAL_ERROR "Required build tools are missing or outdated.\n" "Missing : ${_missing}\n"
                            "Version : ${_ver_err}\n\n" "${_install_guide}"
        )
    endif()
    ...
    set(MOSS_LLVM_BIN_DIR
        "${_llvm_bin_dir}"
        CACHE PATH "LLVM bin directory" FORCE
    )
...
macro(_moss_extract_tool_path json_str tool_name cache_var)
    ...
    set(${cache_var}
        "${_tool_path}"
        CACHE FILEPATH "${tool_name} path" FORCE
    )
endmacro()
