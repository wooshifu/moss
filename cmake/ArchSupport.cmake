...
function(moss_configure_architecture)
    ...
    if(NOT MOSS_TARGET_ARCH)
        message(FATAL_ERROR "必须设置 MOSS_TARGET_ARCH 变量。支持的架构: ${SUPPORTED_ARCHITECTURES}")
    endif()

    # 验证架构支持
    if(NOT MOSS_TARGET_ARCH IN_LIST SUPPORTED_ARCHITECTURES)
        message(FATAL_ERROR "不支持的架构: ${MOSS_TARGET_ARCH}。支持的架构: ${SUPPORTED_ARCHITECTURES}")
    endif()
...
function(moss_configure_clang_toolchain arch)
    ...
    set(CMAKE_ASM_COMPILER_TARGET ${target_triple} PARENT_SCOPE)

    # 架构特定设置
    if(arch STREQUAL "ARM64")
        set(CMAKE_SYSTEM_PROCESSOR "aarch64" PARENT_SCOPE)
    elseif(arch STREQUAL "X86_64")
        set(CMAKE_SYSTEM_PROCESSOR "x86_64" PARENT_SCOPE)
    elseif(arch STREQUAL "RISCV")
        set(CMAKE_SYSTEM_PROCESSOR "riscv64" PARENT_SCOPE)
    endif()
endfunction()
...
function(moss_set_arch_compile_flags arch)
    ...
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${KERNEL_FLAGS_STR}" PARENT_SCOPE)
    set(CMAKE_ASM_FLAGS "${CMAKE_ASM_FLAGS} --target=${ARCH_TARGET_MAP_${arch}}" PARENT_SCOPE)
...
function(moss_configure_platform)
    if(NOT MOSS_TARGET_PLATFORM)
        set(MOSS_TARGET_PLATFORM "QEMU_VIRT" PARENT_SCOPE)
        message(STATUS "未指定平台，使用默认平台: QEMU_VIRT")
    endif()

    # 验证平台支持
    if(NOT MOSS_TARGET_PLATFORM IN_LIST SUPPORTED_PLATFORMS)
        message(FATAL_ERROR "不支持的平台: ${MOSS_TARGET_PLATFORM}。支持的平台: ${SUPPORTED_PLATFORMS}")
    endif()
