...
def generate(program: bytes, child: bytes, signals: bytes, libc_program: bytes, busybox: bytes) -> bytes:
    ...
    return b"".join(
        [
            ...
            make_cpio_entry("signal_test.elf", signals, ino=4),
            make_cpio_entry("libc_validation.elf", libc_program, ino=5),
            make_cpio_entry("busybox.elf", busybox, ino=6),
            make_cpio_entry("bad_entry.elf", bytes(bad_entry), ino=7),
            make_cpio_entry("bad_phentsize.elf", bytes(bad_phentsize), ino=8),
            make_cpio_entry("bad_load.elf", bytes(bad_load), ino=9),
            make_cpio_trailer(),
        ]
    )
...
if __name__ == "__main__":
    output, program, child, signals, libc_program, busybox = map(Path, sys.argv[1:])
    output.write_bytes(
        generate(
            program.read_bytes(),
            child.read_bytes(),
