...
[[noreturn]] void kernel_main(void) noexcept {
  ...
  early_debug_print("🔍 调试：检查MOSS_TEST_MODE宏定义...\n");
    ...
    kernel_run_unit_tests();  // never returns
  #else
...
// ============================================================================
// Unit test framework integration - delegates to ut_kernel.hpp based tests
// ============================================================================

#ifdef MOSS_TEST_MODE

// Forward declaration: test entry point defined in src/test/test/test_main.cpp
extern "C" [[noreturn]] void test_kernel_main() noexcept;

[[noreturn]] void kernel_run_unit_tests(void) noexcept {
    early_debug_print("🧪 Delegating to ut_kernel.hpp test framework...\n");
    test_kernel_main();  // never returns
}
