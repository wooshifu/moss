...
public:
  ...
  // 赋值操作符
  constexpr Result &
  operator=(const Result &other) noexcept(is_nothrow_copy_constructible_v<T> &&
  ...
  [[nodiscard]] constexpr T &&value() && noexcept { return move(value_); }
  ...
  [[nodiscard]] constexpr T &&operator*() && noexcept { return move(value_); }
...
template <typename E> [[nodiscard]] constexpr Result<void, E> Error(E &&error) {
