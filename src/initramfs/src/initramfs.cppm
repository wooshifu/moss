...
inline u32 parse_hex8(const char* s) noexcept {
    ...
    for (int i = 0; i < 8; ++i) {
        val <<= 4;
        char c = s[i];
        if (c >= '0' && c <= '9')      val |= static_cast<u32>(c - '0');
        else if (c >= 'a' && c <= 'f') val |= static_cast<u32>(c - 'a' + 10);
        else if (c >= 'A' && c <= 'F') val |= static_cast<u32>(c - 'A' + 10);
    }
    return val;
}
...
inline usize align4(usize v) noexcept {
    return (v + 3) & ~static_cast<usize>(3);
}
...
public:
    ...
    bool init(PhysAddr base, usize size) noexcept {
        ...
        while (ptr + sizeof(CpioNewcHeader) <= end && file_count_ < MAX_INITRAMFS_FILES) {
            ...
            if (hdr->c_magic[0] != '0' || hdr->c_magic[1] != '7' ||
                ...
                log::klog::warn("initramfs: bad magic at offset {:#x}",
                                static_cast<u64>(ptr - base_));
                break;
            }
        ...
        log::klog::info("initramfs: parsed {} files from {:#x} ({} bytes)",
                        file_count_, base, size);
        for (u32 i = 0; i < file_count_; ++i) {
            log::klog::info("  [{}] '{}' {} bytes mode={:#o}",
                            i, entries_[i].name, entries_[i].data_size, entries_[i].mode);
        }
    ...
    [[nodiscard]] const InitramfsEntry* lookup(const char* path) const noexcept {
        if (!initialized_ || !path) return nullptr;

        // Strip leading "/"
        const char* search = path;
        if (search[0] == '/') search = path + 1;
    ...
    template <typename F>
    void for_each(F&& callback) const noexcept {
        for (u32 i = 0; i < file_count_; ++i) {
            callback(entries_[i]);
        }
    }
