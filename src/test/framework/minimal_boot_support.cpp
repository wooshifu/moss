...
// 早期调试输出 - 测试内核版本
void early_debug_print(const char *message) noexcept { uart_puts(message); }
...
// ARM64架构特定的系统调用入口
#if defined(MOSS_ARCH_ARM64)
[[noreturn]] void handle_syscall() noexcept { syscall_handler(); }
#endif
...
void *operator new[](usize size) { return operator new(size); }
...
// ELF加载器实现
[[nodiscard]] Result<LoadedProgram> ElfLoader::load_elf_from_memory(const u8 * /*elf_data*/,
                                                                    usize /*elf_size*/) noexcept {
