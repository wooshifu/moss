...
template <typename T> using NUMAResult = moss::kernel::Result<T, NUMAError>;
using NUMAVoidResult = moss::kernel::Result<void, NUMAError>;

enum class NUMAPolicy : u32 { DEFAULT = 0, BIND = 1, INTERLEAVE = 2, PREFERRED = 3, LOCAL = 4 };
...
struct NUMANode {
  ...
  NUMANode() noexcept
      : node_id(NUMA_NO_NODE), state(NodeState::OFFLINE), memory_start(0), memory_size(0), free_memory(0),
        allocated_memory(0), cpu_mask(0), cpu_count(0), local_allocations(0), remote_allocations(0), migration_count(0),
        memory_pressure(0), last_balance_time(0) {}

  NUMANode(numa_node_t id, PhysAddr start, usize size) noexcept
      : node_id(id), state(NodeState::ONLINE), memory_start(start), memory_size(size), free_memory(size),
        allocated_memory(0), cpu_mask(0), cpu_count(0), local_allocations(0), remote_allocations(0), migration_count(0),
        memory_pressure(0), last_balance_time(0) {}
};
...
public:
  struct AllocationRequest {
    ...
    AllocationRequest(usize sz, NUMAPolicy pol = NUMAPolicy::DEFAULT, numa_node_t node = NUMA_NO_NODE) noexcept
        : size(sz), alignment(PAGE_SIZE), policy(pol), preferred_node(node), node_mask(~0UL), flags(Flags::NONE) {}
  };
...
public:
  ...
  [[nodiscard]] numa_node_t select_fallback_node(numa_node_t failed_node,
                                                 const AllocationRequest &request) const noexcept;
...
public:
  ...
  [[nodiscard]] NUMAResult<PhysAddr> numa_alloc_pages(usize count, NUMAPolicy policy = NUMAPolicy::DEFAULT,
...
inline NUMAResult<PhysAddr> alloc_pages(usize count, NUMAPolicy policy = NUMAPolicy::DEFAULT) noexcept {
...
inline numa_node_t get_current_node() noexcept { return NUMAPolicyManager::get_instance().get_current_numa_node(); }
inline double get_efficiency() noexcept { return NUMAPolicyManager::get_instance().get_numa_efficiency(); }
inline bool is_available() noexcept { return NUMAPolicyManager::get_instance().is_numa_enabled(); }
} // namespace numa
...
template <typename T> using VmallocResult = moss::kernel::Result<T, VmallocError>;
...
struct VmArea {
  ...
  VmArea() noexcept
      : start(0), end(0), size(0), type(VmAreaType::VMALLOC), permissions(0), numa_node(NUMA_NO_NODE), flags(0),
        parent(nullptr), left(nullptr), right(nullptr), color(RBColor::RED), list_next(nullptr), list_prev(nullptr),
        creation_time(0), access_count(0), ref_count(0) {}

  VmArea(VirtAddr s, usize sz, VmAreaType t, u64 perms) noexcept
      : start(s), end(s + sz), size(sz), type(t), permissions(perms), numa_node(NUMA_NO_NODE), flags(0),
        parent(nullptr), left(nullptr), right(nullptr), color(RBColor::RED), list_next(nullptr), list_prev(nullptr),
        creation_time(0), access_count(0), ref_count(0) {}

  [[nodiscard]] bool contains(VirtAddr addr) const noexcept { return addr >= start && addr < end; }
...
private:
  ...
  static VmallocVoidResult map_vmalloc_pages(VirtAddr vaddr, usize size, u64 permissions, numa_node_t node) noexcept;
...
namespace vmalloc {
[[nodiscard]] inline VmallocResult<void *> alloc(usize size) noexcept { return VmallocAllocator::vmalloc(size); }
...
inline VmallocVoidResult free(void *addr) noexcept { return VmallocAllocator::vfree(addr); }
...
inline VmallocVoidResult free_stack(void *base) noexcept { return VmallocAllocator::free_kernel_stack(base); }
} // namespace vmalloc
