...
public:
  ...
  create_region(ProcessId creator_pid, usize size,
    ...
    // 分配共享内存ID
    ShmId region_id =
        next_shm_id_.fetch_add(1, containers::MemoryOrder::Relaxed);
    ...
    // 创建共享内存区域
    ShmRegion *region =
        new ShmRegion(region_id, phys_addr, virt_addr, aligned_size, type,
                      permission, creator_pid);
    ...
    (void)total_memory_usage_.fetch_add(aligned_size,
  ...
  [[nodiscard]] KernelResult<VirtAddr> map_to_process(
    ...
    // 分配进程虚拟地址
    VirtAddr user_virt_addr =
        allocate_user_virtual_address(pid, region->size, hint_addr);
    if (user_virt_addr == 0) {
    ...
    // 建立用户空间映射
    auto map_result =
        map_user_memory(pid, user_virt_addr, region->phys_base, region->size,
                        region->attributes, region->page_size);
    if (!map_result) {
    ...
    // 记录进程映射
    record_process_mapping(pid, region_id, user_virt_addr, region->size,
                           map_permission);
  ...
  // 从进程地址空间取消映射
  [[nodiscard]] VoidResult unmap_from_process(ProcessId pid,
                                              ShmId region_id) noexcept {
  ...
  [[nodiscard]] VoidResult destroy_region(ShmId region_id) noexcept {
    ...
    (void)total_memory_usage_.fetch_sub(region->size,
                                        containers::MemoryOrder::Relaxed);
  ...
  // 获取共享内存区域信息
  [[nodiscard]] const ShmRegion *
  get_region_info(ShmId region_id) const noexcept {
    auto region_ptr = regions_.find(region_id);
    if (region_ptr == nullptr) {
      return nullptr;
    }
...
private:
  // 选择最优页面大小
  [[nodiscard]] usize select_page_size(usize size,
                                       ShmType type) const noexcept {
  ...
  // 页面对齐
  [[nodiscard]] static usize align_up_to_page(usize size,
                                              usize page_size) noexcept {
    return (size + page_size - 1) & ~(page_size - 1);
  }

  // 获取内存属性
  [[nodiscard]] static mm::MemoryAttributes
  get_memory_attributes(ShmType type) noexcept {
    switch (type) {
    case ShmType::Normal:
  ...
  allocate_physical_memory([[maybe_unused]] usize size,
  ...
  void free_physical_memory([[maybe_unused]] PhysAddr addr,
  ...
  // 虚拟地址分配（简化实现）
  [[nodiscard]] VirtAddr
  allocate_kernel_virtual_address([[maybe_unused]] usize size) noexcept {
    // 实际实现中需要从内核虚拟地址空间分配
    return 0xFFFF800000000000ULL;
  }

  void free_kernel_virtual_address([[maybe_unused]] VirtAddr addr,
                                   [[maybe_unused]] usize size) noexcept {
    // 实际实现中需要释放虚拟地址
  }

  [[nodiscard]] VirtAddr
  allocate_user_virtual_address([[maybe_unused]] ProcessId pid,
  ...
  map_kernel_memory([[maybe_unused]] VirtAddr virt,
  ...
  void unmap_kernel_memory([[maybe_unused]] VirtAddr virt,
                           [[maybe_unused]] usize size) noexcept {
    // 实际实现中需要清除页表项
  }

  [[nodiscard]] VoidResult
  map_user_memory([[maybe_unused]] ProcessId pid,
  ...
  find_process_mapping(ProcessId pid, ShmId region_id) noexcept {
    ...
    const ShmMapping *found =
        mappings->find_if([region_id](const ShmMapping &mapping) {
          return mapping.region_id == region_id;
        });

    return found ? containers::Optional<ShmMapping>{*found}
                 : containers::Optional<ShmMapping>{};
  }
  ...
  void update_page_statistics(usize page_size, i32 delta) noexcept {
    if (page_size == LARGE_PAGE_SIZE) {
      if (delta > 0) {
        (void)large_pages_used_.fetch_add(static_cast<usize>(delta),
                                          containers::MemoryOrder::Relaxed);
      } else {
        (void)large_pages_used_.fetch_sub(static_cast<usize>(-delta),
                                          containers::MemoryOrder::Relaxed);
      }
    } else if (page_size == HUGE_PAGE_SIZE) {
      if (delta > 0) {
        (void)huge_pages_used_.fetch_add(static_cast<usize>(delta),
                                         containers::MemoryOrder::Relaxed);
      } else {
        (void)huge_pages_used_.fetch_sub(static_cast<usize>(-delta),
                                         containers::MemoryOrder::Relaxed);
      }
    }
  }
  ...
  void cleanup_all_regions() noexcept {
    regions_.for_each(
        [this](const auto &entry) { (void)destroy_region(entry.key); });
  }
