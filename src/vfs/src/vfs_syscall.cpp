...
extern "C" [[gnu::weak, gnu::noinline]] void moss_validation_pipe_published(void *) noexcept {}
// Observe copyout while both slots remain reserved, without the FD lock held.
extern "C" [[gnu::weak, gnu::noinline]] void moss_validation_pipe_copyout(void *, long, long) noexcept {}
extern "C" [[gnu::weak, gnu::noinline]] void moss_validation_dup_selected(void *) noexcept {}
...
static long create_node_locked(const char *path, FileType type, u32 mode, u32 uid, u32 gid, Dentry *&created,
  ...
  if (!dir->inode_ops || !dir->inode_ops->create)
    return -static_cast<long>(VfsError::NotSupported);
  long result = dir->inode_ops->create(dir, name, static_cast<u32>(name_length), type, mode);
  if (!result) {
    created = dir->inode_ops->lookup(dir, name, static_cast<u32>(name_length));
...
long do_rename(const char *old_path, const char *new_path, void *fd_table_ptr, u32 uid, u32 gid) noexcept {
  ...
  auto *ops = old_parent->inode->inode_ops;
  if (!ops || !ops->rename)
    return -static_cast<long>(VfsError::NotSupported);
