...
static int file_transfer(struct OpenDescription *description, unsigned long memory, unsigned int count, int writing,
  ...
  unsigned char operation = MOSS_FILE_READ;
  if (writing) {
    operation = description->flags & MOSS_PROCESS_FD_APPEND ? MOSS_FILE_APPEND : MOSS_FILE_WRITE;
  }
...
static void handle_fd_request(struct Record *owner, unsigned long namespace, unsigned long pipe, unsigned long reply,
  ...
  if (operation == MOSS_PROCESS_FD_GET_FLAGS || operation == MOSS_PROCESS_FD_SET_FLAGS) {
    if (request->capability || request->rights ||
        request->size != (operation == MOSS_PROCESS_FD_GET_FLAGS ? MOSS_PROCESS_REPLY_VALUE_BYTES
  ...
  if (operation == MOSS_PROCESS_FD_READ || operation == MOSS_PROCESS_FD_WRITE) {
    ...
    int result;
    if (description->kind == DESCRIPTION_PIPE) {
      result = pipe_transfer(description, request->capability, count, writing, &transferred);
    } else if (description->kind == DESCRIPTION_CONSOLE) {
      result = console_transfer(description, request->capability, count, writing, &transferred);
    } else {
      result = file_transfer(description, request->capability, count, writing, &position, &transferred);
    }
    if (result == 1) {
      ...
      if (result == BACKEND_WOULD_BLOCK) {
        response->payload[0] = MOSS_PROCESS_WOULD_BLOCK;
      } else if (result == BACKEND_BROKEN_PIPE) {
        response->payload[0] = MOSS_PROCESS_BROKEN_PIPE;
      } else if (result == BACKEND_NO_SPACE) {
        response->payload[0] = MOSS_PROCESS_NO_SPACE;
      } else {
        response->payload[0] = MOSS_PROCESS_UNAVAILABLE;
      }
      if (result < 0 && writing) {
  ...
  if (operation == MOSS_PROCESS_FD_SEEK) {
    if (request->size != MOSS_PROCESS_FD_SEEK_BYTES || request->capability || request->rights ||
  ...
  for (;;) {
    ...
    if (request.size == 1 &&
      ...
      if (!child || (child->parent_id != request.badge && child->id != request.badge)) {
        ...
        if (syscall2(SYS_DOMAIN_SAME, (long)request.capability, (long)request.capability) != 1 ||
      ...
      if (!record_running(caller) || !target || (target != caller && target->parent_id != caller->id)) {
        ...
        if (signaled == 0) {
          response.payload[0] = MOSS_PROCESS_OK;
        } else if (signaled == -ESRCH) {
          response.payload[0] = MOSS_PROCESS_NO_ENTRY;
        } else if (signaled == -EINVAL) {
          response.payload[0] = MOSS_PROCESS_BAD_REQUEST;
        } else {
          response.payload[0] = MOSS_PROCESS_UNAVAILABLE;
        }
