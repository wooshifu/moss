...
static int fd_stat(unsigned long session, unsigned long number, unsigned char *kind, unsigned long *id,
  ...
  struct moss_ipc_message response = {0};
  long result = call(session, &request, &response);
  if (!no_capability(&response) || result != MOSS_PROCESS_FD_STAT_REPLY_BYTES ||
...
static int fd_view_probe(void) {
  ...
  if (valid) {
    ...
    if (valid) {
      ...
      struct moss_ipc_message truncate = {
          .size = 2 + sizeof("/busybox.elf"),
          .payload = {MOSS_PROCESS_FD_OPEN, MOSS_PROCESS_FD_WRITABLE | MOSS_PROCESS_FD_TRUNCATE}};
...
static int fd_pipe(unsigned long session, unsigned long *reader, unsigned long *writer) {
  ...
  struct moss_ipc_message response = {0};
  long result = call(session, &request, &response);
  if (!no_capability(&response) || result != MOSS_PROCESS_FD_PIPE_REPLY_BYTES ||
      response.payload[0] != MOSS_PROCESS_OK) {
    return 0;
  }
...
static int fd_wait_expires(unsigned long session, unsigned long number) {
  ...
  if (syscall2(SYS_CLOCK_GETTIME, MOSS_CLOCK_MONOTONIC, (long)&now) != 0 ||
      now > LONG_MAX - fd_wait_cancel_timeout_ns) {
    return 0;
  }
  struct moss_ipc_message response = {0};
