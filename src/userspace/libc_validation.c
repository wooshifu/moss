...
static int uname_runtime(void) {
  ...
  for (unsigned i = 0; i < 6; ++i) {
    const char *field = (const char *)&result.identity + i * 65;
...
static int working_directory_runtime(void) {
  ...
  if (chdir("/dev") || !getcwd(cwd, sizeof(cwd)) || strcmp(cwd, "/dev") || chdir(".././dev/..") ||
    ...
    return 8;
  if (mkdir("/cwd-sandbox", 0755) || chdir("/cwd-sandbox") || !getcwd(cwd, sizeof(cwd)) || strcmp(cwd, "/cwd-sandbox"))
    return 2;
...
static int pipe_runtime(void) {
  ...
  if (duplicate < 0 || dup2(duplicate, input) != input || dup2(input, input) != input)
    return 0;
  if (fcntl(input, F_GETFL) != O_RDONLY || fcntl(output, F_GETFL) != O_WRONLY || fcntl(duplicate, F_GETFL) != O_RDONLY)
    return 0;
  errno = 0;
