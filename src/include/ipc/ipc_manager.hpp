...
struct ConnectionDescriptor {
    ...
    ConnectionDescriptor(ChannelId cid, ProcessId client, ProcessId server, ServiceId sid) noexcept
        : channel_id(cid), client_pid(client), server_pid(server), service_id(sid),
          established_time(0), last_activity(0), messages_sent(0),
          messages_received(0), bytes_transferred(0) {}
};
...
public:
    ...
    // 注册服务
    [[nodiscard]] KernelResult<ServiceId> register_service(ProcessId provider_pid,
    ...
    [[nodiscard]] KernelResult<ChannelId> connect_to_service(ProcessId client_pid,
        ...
        // 创建连接描述符
        ConnectionDescriptor* conn = new ConnectionDescriptor(channel_id, client_pid,
    ...
    [[nodiscard]] bool wait_for_message(ChannelId channel_id, ProcessId receiver_pid,
                                       MessageHeader& header, void* payload,
                                       usize max_payload_size, u64 timeout_ns = UINT64_MAX) noexcept {
        // 查找通道
        const auto* channel = channels_.find(channel_id);
        if (channel == nullptr || !*channel) {
            return false;
