...
struct InterruptDescriptor {
    ...
    InterruptDescriptor(InterruptId id, InterruptHandler h, void* ctx, const char* n) noexcept
        : irq(id), type(determine_type(id)), trigger(TriggerType::LevelHigh),
          priority(128), target_cpu_mask(1), handler(h), context(ctx),
          count(0), enabled(false), name(n) {}
...
public:
    ...
    // 注册中断处理函数
    [[nodiscard]] VoidResult register_interrupt(InterruptId irq,
    ...
    [[nodiscard]] GicStats get_statistics() const noexcept {
        ...
        return {
            total_interrupts_.load(containers::MemoryOrder::Relaxed),
