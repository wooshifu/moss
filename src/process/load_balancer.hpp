...
public:
    ...
    [[nodiscard]] LoadBalanceStats get_stats(u32 cpu) const noexcept {
        if (cpu >= MAX_CPUS) return LoadBalanceStats{};
...
private:
    ...
    // 计算迁移收益
    [[nodiscard]] u64 calculate_migration_benefit(Thread* thread, u32 src_cpu, u32 dst_cpu, CfsScheduler& scheduler) const noexcept {
        if (thread == nullptr) return 0;
