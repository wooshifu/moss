...
struct alignas(16) CpuContext {
    ...
    // 初始化上下文
    constexpr CpuContext() noexcept : x{}, sp(0), pc(0), pstate(0),
                                     fpsr(0), fpcr(0), v{}, tpidr_el0(0) {}
};
...
struct SchedEntity {
    ...
    SchedEntity() noexcept : vruntime(0), exec_start(0), sum_exec_runtime(0),
                           prev_sum_exec_runtime(0), weight(1024), nice(0),
                           prio(120), load_weight(1024), load_sum(0),
                           util_sum(0), load_avg(0), util_avg(0) {}
};
...
struct RtSchedEntity {
    ...
    RtSchedEntity() noexcept : priority(Priority::DEFAULT_RT_PRIORITY),
                              runtime(0), deadline(0), period(0) {}
};
...
struct Thread {
    ...
    Thread(ThreadId id, ProcessId pid) noexcept
        : tid(id), owner_pid(pid), context{}, cpu(0), wake_cpu(0),
          state(ProcessState::Created), sched_class(SchedClass::Normal),
          se{}, rt{}, start_time(0), utime(0), stime(0),
          stack_base(0), stack_size(0), wait_queue(0),
          signal_mask(0), pending_signals(0) {}
};
...
public:
    Process(ProcessId pid, ProcessId parent = INVALID_PROCESS_ID) noexcept
        : pid_(pid), parent_pid_(parent), address_space_(nullptr),
          ...
          state_(ProcessState::Created), exit_code_(0),
          limits_{}, stats_{}, ref_count_(1) {}
    ...
    Process(Process&& other) noexcept
        : pid_(other.pid_), parent_pid_(other.parent_pid_),
          ...
          state_(other.state_), exit_code_(other.exit_code_),
          limits_(other.limits_), stats_(other.stats_),
          ref_count_(1) { // 重置引用计数
...
public:
    ...
    void for_each_process(Func&& func) const {
        processes_.for_each([&func](const auto& entry) {
