...
export module moss.drivers.irqchip;
...
export namespace moss::kernel::drivers::irqchip {

[[nodiscard]] inline VirtAddr plic_context_base(u32 cpu) noexcept {
  // PLIC supervisor threshold/claim windows use the firmware's context index.
  return platform::intc_dist_base() + 0x200000 + platform::hardware.plic_contexts[cpu] * 0x1000ULL;
}
...
inline void enable_secondary_interface() noexcept {
  // Intel SDM: SVR bit 8 enables the APIC; vector 0xff is reserved for spurious IRQs.
  write_reg(platform::intc_dist_base(), cpu_regs::SVR, 0x1FF);
  write_reg(platform::intc_dist_base(), cpu_regs::TPR, 0);
}
inline void send_startup_ipi(u32 apic_id, u32 command) noexcept {
  write_reg(platform::intc_dist_base(), cpu_regs::ICR_HIGH, apic_id << 24);
  write_reg(platform::intc_dist_base(), cpu_regs::ICR_LOW, command);
}
#endif
...
} // namespace moss::kernel::drivers::irqchip
