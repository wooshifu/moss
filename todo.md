...
> 源码复核基线：2026-09-23，`604fe85`（当次未重跑 QEMU）。之后的当前九预设运行验收见 [moss-todo.md](moss-todo.md) 第 3.59～3.73 节。历史实现与实测保留原日期；源码与旧报告的对应关系见第 3.41 节。
...
- [ ] **MOSS-020（部分修复）**：统一 syscall/IRQ 返回信号检查；结果/handler 参数写回、SIGCHLD 基本投递、pipe/console、wait 和 nanosleep 信号中断已有覆盖；SA_RESTART 限定重启子集、CPU-bound IRQ 返回投递、基本 STOP/CONT 交接、SA_NOCLDSTOP、SA_NOCLDWAIT/显式 SIG_IGN 自动回收、进程组 wait 筛选和成员变更唤醒及 waitpid 停止/继续与信号致死状态报告已验收，继续补完整 job control 与更广交错。
