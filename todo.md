...
> 当前源码复核：2026-09-23，`604fe85`；该次文档复核未重跑 QEMU。之后的当前九预设运行验收见 [moss-todo.md](moss-todo.md) 第 3.59～3.61 节。历史实现与实测保留原日期；源码与旧报告的对应关系见第 3.41 节。
...
- [x] 架构预设共 9 个：`{arm64,riscv64,x64}-{debug,release,relwithdebinfo}`；每个 workflow 为 configure → build → matching CTest preset（`b57422d`）。3.40 的九 preset **43/43 CTest** 及 16 CPU/Sv39/GICv3 同镜像补验是当时记录；本轮九预设重新通过 **43/43 CTest**，见 3.61。3.34～3.40 的初始失败、默认超时与受控变异失败仍保留。
