...
> IPC deadline 继承复核：2026-09-24，嵌套调用按当前调用者的最早绝对 deadline 截止；验证范围与在途调用的限制见 [3.100](moss-todo.md#3100-同步-ipc-嵌套调用的-deadline-继承2026-09-24)。
...
> 启动文件与服务交接整合复核：2026-09-24，九预设 **53/53 CTest**、九份生产启动报告各 **165 步**；静态 libc 镜像从只读 File Service 复制并封存，Loader 子域在恢复 scope 中登记，见 [3.98](moss-todo.md#398-启动文件与-loaderprocess-交接整合2026-09-24)。
> Loader 入站封存门禁复核：2026-09-24，主线阶段九预设 **53/53 CTest**、11 份生产启动报告各 **113 步**；与启动归档及受管子进程交接合并后再通过 **53/53 CTest**、九份生产启动报告各 **165 步**。Loader 在读取前自行封存，公开具名文件经真实 `RUN` 被拒绝，见 [3.99](moss-todo.md#399-loader-入站文件封存门禁2026-09-24)。
> 具名文件快照装载复核：2026-09-24，与只读启动归档和原生服务路径合并后，九预设 **53/53 CTest**、九份生产启动报告各 **165 步**；根端点按名生成已封存快照，源文件继续可写，Loader 在源文件被改坏时仍从快照启动，见 [3.101](moss-todo.md#3101-具名文件的不可变装载快照2026-09-24)。
> 原生描述符容量错误复核：2026-09-24，九预设 **53/53 CTest**、九份生产启动报告各 **165 步**；File Service 内容配额拒绝沿 Process Service 保留为 `NO_SPACE`，受管状态映射为 `ENOSPC`，见 3.102。
> Process Service 在途 Reply 恢复复核：2026-09-25，九预设 **53/53 CTest**、九份生产启动报告各 **167 步**；真实 `FD_WAIT` 经 Process Service 把 Reply 转交 Console Service 后，接收端死亡不提前结束调用，持有者退出才返回 `EPIPE`，见 3.103。
...
MOSS 当前是 C++26 freestanding 内核，三架构已有实际启动、中断、调度及用户态执行路径。内核仍承担 POSIX 进程、普通 ELF、VFS 和启动驱动；独立的用户态 supervisor、文件/命名空间服务、Code Authority Service、受限 Loader Service 和 Process Compatibility Service 已经通过 capability 控制 IPC 协作。文件服务已从只读启动归档供应启动文件，并承载 `/scratch` 易失文件。Loader 可从已封存的不具名文件装载带有界启动参数及静态 libc TLS 模板的原生域，再由 supervisor 将恢复 scope 内的域登记为 Process Service 子进程；普通 shell 文件路径和 POSIX `execve` 尚未迁出内核。
...
- [x] 服务线程发起嵌套同步 IPC 时，以本次显式 deadline 和当前活跃调用者的最早绝对 deadline 为界；无显式截止时间的下游调用也随上游超时，迟到 Reply 被拒绝。只保证调用开始时的快照，后来新增的更早 deadline 不追溯缩短在途调用（ADR-0012/0023 的限定传播切片，见 3.100）。
...
- [x] 独立 Loader Service 从 capability 寻址的易失文件读取有界静态 ELF，请求代码批准后经执行域工厂启动普通原生域；有效镜像退出码、畸形镜像拒绝及服务死亡后重启已在生产启动路径验证。启动归档已作为只读 Memory Object 交给 File Service，私有 Loader 镜像从该服务复制；内核 VFS 仍处理 POSIX `execve`。静态 libc TLS 模板另见 3.96；动态链接、产品代码策略与完整迁移继续开放（ADR-0025 的限定切片，见 3.90）。
- [x] Loader 新建的原生域可由私有 supervisor 以 `OBSERVE|INSPECT|SIGNAL` 削减权限登记为 Process Compatibility Service 的受管子进程；带 badge 的会话完成父子身份、退出状态及等待检查，并在每个服务 incarnation 重新验证。普通 POSIX 进程身份和 `execve` 仍未迁出内核（ADR-0024/0025 的限定交接，见 3.93）。
...
- [x] Loader 从能力寻址的易失文件装载超过旧 64 KiB 限制的静态 mlibc ELF；`PT_TLS` 文件模板须由匹配的 `PT_LOAD` 承载，静态运行时可读取初始化与零初始化线程变量。supervisor 每次启动 Loader 都等待探针退出码 41；三架构九预设 53/53 CTest、11 份生产启动报告各 113 步为 3.96 阶段记录。文件服务总容量限 16 MiB，普通 `execve` 尚未迁移（ADR-0025 的静态 libc 切片，见 3.96）。
- [x] supervisor 将私有不具名镜像写入 File Service 后，在交给 Loader 前执行不可逆 `SEAL`；同一对象后续通过任何发送端的 `WRITE`、`APPEND`、`RESIZE` 均被拒绝，启动时以真实 IPC 尝试破坏镜像并检查拒绝。公开具名文件保持可写；持久文件、pager 与普通 POSIX `execve` 的内容身份仍待迁移（ADR-0025/0026 的有界镜像身份切片，见 3.97）。
- [x] Loader 对每个 `RUN` 附带的不具名 File Object 在首次读取前自行执行 `SEAL`，精确校验回执；无法封存则不读取、不申请代码批准、不创建域。每次服务启动时用公开 `/scratch` 的具名句柄验证 `NO_IMAGE` 拒绝，避免未来新调用方绕过 supervisor 的封存步骤；普通 POSIX `execve`、持久文件及恶意文件服务仍待迁移（ADR-0025/0026 的 Loader 入站门禁切片，见 3.99）。
- [x] File Service 根发送端可按名称把具名文件复制为不具名、已封存快照；普通具名 `SEND` 句柄不能申请快照。supervisor 每个文件服务存活期只保留一份，原文件头被写坏期间 Loader 仍从快照启动；合并后九预设 53/53 CTest、九份生产启动报告各 165 步。对象回收、完整资源计费、持久文件及普通 POSIX `execve` 仍开放（ADR-0025/0026 的限定内容身份切片，见 3.101）。
- [x] File Service 内容配额拒绝写入、追加和扩容时返回独立 `NO_SPACE`，Process Service 保留此状态，受管 mlibc 的状态映射对应 `ENOSPC`；生产描述符探针验证失败后的长度、原字节和共享 offset。受管 mlibc 描述符尚未改用服务路径，对象数配额及不确定写入仍开放（ADR-0018 的限定错误语义切片，见 3.102）。
- [x] 实际 Process Service 的 `FD_WAIT` 转交一次性 Reply 到 Console Service 后，生产故障注入确认调用在 Process Service 死亡及旧 shell scope 排空后仍等待，Console Service 退出才以 `EPIPE` 结束；九预设 53/53 CTest、九份生产启动报告各 167 步。孤儿退出记录与服务死亡的并发交错仍待验收（ADR-0011/0012/0024 的限定恢复切片，见 3.103）。
...
- [ ] CPU 预算、在途调用的动态 deadline 收紧、最大调用链深度及死锁策略仍需单独设计和验收（ADR-0012/0023）。
  - [x] CPU 预算的计时基础：三架构调度器统一使用校准后的纳秒时钟，RT/CFS 在线程运行 tick 与离开 CPU 时累计调度占用时间；CFS 公平性计数仍独立。合成时间回归及真实 QEMU 范围见 3.104。预算范围、补充、超额处理和权能准入仍未实现。
