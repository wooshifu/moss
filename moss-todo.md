...
2026-09-24 的代码已经有进程局部 capability、同步控制 IPC、共享内存对象、用户态 supervisor、处理 `/scratch` 的文件/命名空间服务，以及由启动域工厂 capability 授权的原生域构造。独立 Code Authority Service 接受 supervisor 与受委托 Loader 的私有批准请求；Loader 可从不具名文件装载带有界启动参数的静态原生域，supervisor 可将该域的观察权交给 Process Compatibility Service。普通 shell 文件、POSIX `execve` 的 ELF 加载、进程/信号和启动设备仍依赖内核路径；这些限定服务与机制不能视为 ADR-0015、0018、0024～0027 已完成。后续按已接受的 ADR 逐步迁移，并继续验证现存路径的安全与所有权边界。
...
启动时 supervisor 对自身文本页做一次快照并通过真实 IPC 请求批准，保留返回的实例作为探针。服务进程退出后，supervisor 用独立的撤销句柄撤销该存续实例，关闭旧端点，再显式把批准权交给新服务，重建端点并再次请求批准。管理 shell 随后刷新它所持有的服务域终止句柄；文件、命名空间和进程服务保持原 incarnation。生产启动回归用 `moss-domain.elf terminate code` 触发这一过程，核对新服务 PID、shell 恢复及已有文件内容继续可读。三架构九预设串行 workflow **53/53 CTest** 全部通过，九份生产启动结果均完成 92 步并确认代码服务恢复。
...
### 3.98 Loader 入站文件封存门禁（2026-09-24）
...
### 3.99 同步 IPC 嵌套调用的 deadline 继承（2026-09-24）
...
### 3.100 具名文件的不可变装载快照（2026-09-24）

此前 File Service 只允许不具名对象 `SEAL`；Loader 因此必须拒绝具名文件，无法从可继续写入的源文件取得稳定镜像。现在只有持有文件服务根发送端的调用者可按名称请求 `SNAPSHOT`；普通具名文件 `SEND` 句柄不能凭单个对象反复创建副本。File Service 在单接收循环内复制当前长度和字节，创建新的不具名、已封存对象并转交发送端。原文件保持可写，快照的 `WRITE` 和 `RESIZE` 被拒绝。复制计入现有 16 MiB 内容总额和 16 个对象上限；若回复交接失败，刚创建的快照从对象表和配额中回滚。Namespace Service 的 `CREATE` 仍能申请具名对象，完整客户端资源计费并未实现。

supervisor 启动时从内核启动文件复制一份具名 ELF 到易失 File Service，经根发送端按名称重新打开；验证普通具名发送端不能申请快照后，由根发送端只生成一次快照。快照句柄随 File Service 存续，供每次 Loader 重启复用，避免当前没有对象回收协议时耗尽对象槽位。每次 Loader 启动验收先确认快照拒绝写入，再把原具名文件的 ELF 头改坏；Loader 仍从快照启动探针并得到预期退出码，随后恢复源文件头。原来的直接具名 `RUN` 拒绝和三份私有镜像封存检查仍保留。这个路径还没有把普通 POSIX `execve`、持久文件、pager、动态链接或产品代码审批迁出内核。

验收：三架构 Debug、Release、RelWithDebInfo 九预设 **53/53 CTest**，11 份生产启动报告均为 `passed`、各完成 **113 步**；每次 Loader 启动须先完成上述真实 IPC 的权限、封存及源文件破坏后装载检查，生产启动用例还覆盖 Loader 换代。只补新增分支大括号后，最终源码又通过 x64 Debug **6/6 CTest** 与 **113 步**生产启动。宿主 `scripts/tests` **380 passed、1 skipped**；改动 C 文件及协议头的 clang-format、`git diff --check`、新增行 clang-tidy 均通过。全文件 clang-tidy 仍有仓库既存诊断。此结果来自 QEMU 和宿主，不证明恶意文件服务、持久存储或真实硬件上的镜像身份。
