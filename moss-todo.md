...
| MOSS-006 | P0 | 容器与使用者的所有权 | 部分修复；伪 RCU 已替换为 LockedList/LockedHashMap，持有读者与双 CPU 交错有实测；启动设备已通过 DeviceManager/BootDriver 静态注册并激活；IRQ 注销等待已进入回调（3.52），IPI 部分注册失败会回滚（3.53）；实际硬件回调、动态解绑和 IPC 复合生命周期仍待验收。 |
...
**2026-09-23 更新：部分验收。** IRQ 描述符新增回调准入/注销同步：注销返回后，之前已取得描述符的回调不再持有注册者借出的 `context`；双 CPU 确定性交错和去同步红例见 3.52。硬件 IPI 部分注册失败会按已取得所有权回滚，见 3.53。实际硬件 IRQ 回调、动态驱动解绑及 IPC 创建/销毁的复合生命周期尚未闭合。
...
**2026-09-23 源码复核：部分修复，见 3.19、3.41。** handler 参数/返回值已有回归；默认 `users.signals` 已包含 SIGCHLD 基本断言、pipe/console 中断与部分传输。CPU-bound 的 IRQ 返回投递、STOP/CONT 和 wait/nanosleep 中断结果仍待闭合。
...
- [ ] A4：替换错误的自动退休容器；先用明确锁与拥有关系闭合生命周期（006）。
  ...
  - [ ] 闭合所有使用者的复合操作、外部对象生命周期及跨锁顺序：实际硬件 IRQ 回调、驱动绑定/解绑、IPC 创建/销毁/清理等；补针对这些协议的交错验收。
...
- [ ] D5：基本 SIGCHLD、pipe/console 中断已有用例；补 CPU-bound 的 IRQ 返回投递、STOP/CONT 和 wait/nanosleep 中断语义（020）。
