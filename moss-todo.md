...
> 源码复核基线：2026-09-23，`abeba591`（当次未重跑 QEMU）。第 3.4～3.20 节保留此前实测及当时状态；后续实现与运行记录见第 3.21～3.101 节，源码与历史证据的对应关系见第 3.41 节。前一轮九预设运行见第 3.59～3.75 节。本文与 [todo.md](todo.md) 同步。
> ADR 迁移复核基线：2026-09-24，`ded02f6` 加后续实现。第 3.75 节以前的测试数值不自动覆盖新服务路径或 ADR 收敛改动；IPC 分界见 3.76，启动驱动边界见 3.77，优先级继承见 3.78，取消与服务死亡交错见 3.79，已认领请求的 deadline 见 3.80，reply/取消竞争见 3.81，执行域构造机制见 3.82，不可变代码版本见 3.83，多页代码范围见 3.84，可信版本长度见 3.85，优先级倒置时延见 3.86，代码批准实例撤销见 3.87，批准服务死亡后的权能存续见 3.88，真实审批服务与监督者交接见 3.89，普通原生域的用户态装载见 3.90，Reply capability 跨进程交接见 3.91，失败回滚见 3.92，Loader/Process Service 交接见 3.93，质量门禁参数长度见 3.94，Loader 启动参数见 3.95，静态 libc 镜像与 TLS 模板见 3.96，私有镜像封存见 3.97，启动文件与交接整合见 3.98，Loader 入站文件封存见 3.99，同步 IPC deadline 继承见 3.100，具名文件的不可变装载快照见 3.101，文件内容配额错误见 3.102，当前目标与进度另见 [todo.md 的 ADR 迁移清单](todo.md#adr-迁移)。
> 第 3.1～3.2 节保留原始运行证据；第 5～9 节未标新日期的“位置与事实”、地址和行号属于原始审计，不能当作当前仍然失败的运行结果。带日期的“工作区”指当时的验证状态，不表示当前仍未提交；当前状态以第 4 节及各项更新说明为准。

## 1. 当前结论

Moss 已具备三架构真实启动、SMP、用户态与进程执行路径。后续提交 `f7768373` 建立了生产内核验证和函数基准；`83f5fa94` 实现了同 ISA 通用镜像、运行时硬件发现与独立 runner；`0a7f1d7c` 恢复了全部 9 个 CMake workflow 的 CTest 步骤。RISC-V 64/x64 不是启动桩，旧 x86 测试 ELF 装载失败和 initramfs fallback 也不再代表当前正常路径。

但“能启动、能完成一次 fork/exec/wait”仍不等于可靠的多架构研究内核：

1. **隔离缺陷尚未全部闭合。** 已修复内核映射 USER，并收紧三架构最终内核 W^X；syscall 0 原始 UART 指针旁路已删除，用户地址域/VMA 准入与跨 VMA 权限校验已补。`3461c749` 改为原生 TrapFrame、按 ISA 净化信号返回状态；3.20～3.34 补共享用户复制、受控 COW OOM、地址空间持有、软件 VM 事务和同步页租约。3.35～3.39 补三架构 TLB 失效、活动硬件 root 拥有权及首次 CPU 注册交错；3.40 已关闭原生信号帧/备用栈的恶意输入验收。共享 exec 的线程/root 协调、异步长期页 pin 和更多并发 VM 交错仍未闭合。
2. **部分基础修复已落地，但所有权验收未完成。** `e7fe8c91` 已验证 heap/PFA 对齐与释放、保留洞和多 bank 耗尽、当前布局及页表/元数据哨兵。`c895c876` 修复指针发布误删可达节点；伪 RCU 已删除，锁保护的拥有型容器已有持有读者、重入与双 CPU 交错测试。IRQ/驱动/IPC 复合生命周期、页引用并发及完整启动保留集合仍待完成。
...
2026-09-24 的代码已经有进程局部 capability、同步控制 IPC、多页共享内存对象、用户态 supervisor、进程兼容服务、处理启动归档和 `/scratch` 的平坦文件/命名空间服务，以及由启动域工厂 capability 授权的原生域构造。独立 Code Authority Service 接受 supervisor 与受委托 Loader 的私有批准请求；Loader 可从不具名文件装载带有界启动参数的静态原生域，supervisor 可将进入当前恢复 scope 的域登记为兼容服务子进程。普通 shell 文件、POSIX `execve` 的 ELF 加载和启动设备仍依赖内核路径；POSIX 进程/信号仍有内核兼容路径。这些限定服务与机制不能视为 ADR-0015、0018、0024～0027 已完成。后续按已接受的 ADR 逐步迁移，并继续验证现存路径的安全与所有权边界。
...
原审计行号指 `d86077b2`，初次更新说明中的符号和路径按 `0a7f1d7c` 核对；后续提交与各轮当时的工作区修复另标。请用“路径 + 符号 + 不变量”定位。第 3 节按日期记录已执行的验收，3.41 只做源码复核；不能把普通用例或旧报告外推成当前 HEAD 的全部验收通过。`[x]` 只关闭该行限定的子任务，`[ ]` 包括待实现、部分实现及待专项验收。
...
2026-09-06 复核先比较 `d86077b2..0a7f1d7c`，再对相关符号、调用链和当时源码进行检查。知识图谱使用 Tier 2，初始 generation 为 `2026-09-06T10:43:22Z`；已检查证据路径覆盖，并直接读取汇编、链接脚本及部分解析范围。对于图谱未捕获的全限定 C++ 方法，使用直接源码确认。该轮没有重新执行全部历史故障实验；2026-09-23 的源码复核范围另见 3.41。
...
### 3.3 实施前的复核证据（2026-09-06，0a7f1d7c）
...
命令：对表中每个 preset 执行 `uv run cmake --build --preset <preset> -j4` 和 `uv run ctest --preset <preset>-test --output-on-failure`；Release CTest 串行执行，避免基准 guest 互相竞争。日志分别为 `build/<preset>/smp-ready-build.log`、`smp-ready-ctest.log`；全部 15 个 CTest 入口通过。报告路径仍为 `build/<preset>/validation/<ID>/results.json`，报告中的 revision 是改写前的历史值（当前对应提交为 `0a7f1d7c`），dirty 为 true，镜像 hash 和工具版本保留在报告中。
...
命令：`uv run cmake --build --preset <preset> -j4`、`uv run ctest --preset <preset>-test --output-on-failure`。构建日志为 `build/<preset>/pfa-firmware-build.log`，CTest 日志为 `pfa-final-ctest.log`；15 个 CTest 入口全部通过。默认功能当前为 resources/mm/pfa/heap/vfs/users 六套件、14 用例。报告保留改写前的 revision（当前对应提交为 `0a7f1d7c`）、dirty=true 及实际 hash，路径为 `build/<preset>/validation/<ID>/results.json`：
...
接续 `c895c876`，生产容器改为复用现有 IrqSpinLock 的 LockedList/LockedHashMap；删除 RcuReadLock、回调队列/池和 RCU 容器，不保留同名兼容层。节点由容器拥有，查找返回 Optional 值副本；共享对象使用现有 shared_ptr 保持查找后的生命周期。发布、查找、摘除在锁内，节点析构在锁外；scoped for_each/update_if 不允许阻塞、重入或外逸引用，可能重入的调用者使用锁外 for_each_snapshot。快照为 O(n) 临时复制，不引入新的回收框架。
...
未修改 `38aa678f` 内核时，ARM64 Debug `containers.smp` 连续运行前 24 次通过，第 25 次在测试开始前发生 `unexpected_kernel_panic`；报告 `build/arm64-debug/validation/1789049114268692000/results.json` 的 ready 为空、用例为 not_run。不能把它归因于 waitpid，因为用户测试还没有运行。
...
### 3.19 原生 TrapFrame、fork 与信号返回（2026-09-14，`3461c749`）
...
**提交前矩阵：** `uv run build.py --jobs 3`：九个 configure/build/test workflow 全通过（162.1 s），CTest **21/21**；宿主 `uv run pytest -q scripts/tests` **121/121**，Ruff 与 diff 检查通过。原始报告的 revision 保持改写前的运行时值（当前对应提交为 `16e51d5f`）、dirty=true，不回写成提交后的 SHA；对应源码随后提交为 `3461c749`。各报告路径为 `build/<preset>/validation/<ID>/results.json`：
...
### 3.20 用户复制异常恢复（2026-09-14，基于 `3461c749`）
...
**提交前回归：** `NO_COLOR=1 TERM=dumb uv run build.py --jobs 3` 九个 configure/build/test workflow 全通过（158.0 s），CTest **21/21**；每个配置的默认功能包含 `users.uaccess`。宿主 `uv run pytest -q scripts/tests` **121/121**；全仓格式检查、新增模块文件的 clang-format 检查、runner Ruff 检查和 diff 检查通过。原始报告均基于 `3461c749`、dirty=true；本轮通过不取消 3.18 的间歇失败记录。
...
### 3.21 VFS/信号安全复制与部分 I/O（2026-09-14，基于 `c74c3e12`）
...
### 3.22 x64 PVH initrd 契约与启动失败闭合（2026-09-19，基于 `915ebdbf`，工作区）
...
六个受影响 Debug/Release 构建均成功。下表报告均 finalized，取证时共同记录改写前的 revision（当前对应提交为 `915ebdbf6abfd5fdd894e2a2e99e30beebe3aeb0`）、dirty=true 与 source hash `af2d1442f941a79e901a0e3cbf5f5542cf93ba4f1df126d4e89c9c6aa2470531`；随后只更新了本节等交付文档，因此该哈希不是文档完成后的工作区身份：
...
修复后六个 Debug/Release 构建全部成功。3.26 的 ELF LoadPlan 验收落地后又以最终生产源码重跑：每个 preset 的 finalized 报告都包含 `users.vm` 四项、`mm.permissions` 七项、`mm.transactions` 九项、`users.exec` 二十八项及 libc/BusyBox 消费者，共 **6 套件 / 73 case**，全部通过。报告在执行时共同记录改写前的 revision（当前对应提交为 `915ebdbf6abfd5fdd894e2a2e99e30beebe3aeb0`）、dirty=true 和 source hash `c936891a8035d9aaaaead297cd85f5ab002b3dc07cd865edfad75804503cfe8b`；交付文档随后更新，因此该哈希是被执行源码身份，不是文档完成后的工作区哈希。
...
最终六个 Debug/Release 构建均成功。以下串行报告各含 **6 套件 / 73 case**：`users.exec` 28 项、`users.vm` 4 项、`mm.permissions` 7 项、`mm.transactions` 9 项、`users.libc` 2 项和 `users.busybox` 23 项；全部通过、无 not_run，并共同记录改写前的 revision（当前对应提交为 `915ebdbf6abfd5fdd894e2a2e99e30beebe3aeb0`）、dirty=true、source hash `c936891a8035d9aaaaead297cd85f5ab002b3dc07cd865edfad75804503cfe8b`：
...
### 3.28 MM 初始化发布与 affinity 返回契约（2026-09-20，基于 `eea4752e`，工作区）
...
### 3.29 MOSS-030 能力契约闭合与 applications 前进性修复（2026-09-20，基于 `eea4752e`，工作区）
...
### 3.30 MOSS-001 用户态内核映射隔离验收（2026-09-20～21，基于 `0cab79d1`，工作区）

生产页表的 supervisor-only、W^X 和内核共享子树修复提交为 `38aa678f`，本轮已核对其 `set_table` 用户权限参数及 `build_kernel_entry` 按链接段选择 KERNEL_RX/KERNEL_RO 的实际 diff；本节补齐该实现剩余的受控用户异常验收。
...
### 3.31 COW OOM 的复制恢复与共享页生命周期验收（2026-09-21，基于 `0cab79d1`，工作区）

生产修复 `c74c3e12` 已使 `try_cow_fault` 在分配失败时返回异常入口，由原生用户异常终止进程或由共享 uaccess fixup 返回复制失败。本轮补齐其 COW OOM 专项验收，并在失败分支旁注明不变量：分配成功前不得放开原 PTE 写权限、递减共享页引用或改变数据。没有替换 allocator、手工伪造页引用或给生产 syscall 增加测试控制。
...
### 3.32 地址空间持有读者、替换与控制块 OOM（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.33 地址空间软件事务与双 CPU 缺页竞争（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.34 uaccess 地址空间绑定与同步页租约（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.35 ARM64 按地址 TLB 广播与活动 root 回归（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.36 x64/RV64 同步 TLB shootdown 与 IRQ 等待互等修复（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.37 双发布者 TLB 请求交错验收（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.38 活动硬件页表根的拥有权与退休（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.39 首个辅助 CPU 的 TLB 注册与请求交错（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.40 信号帧与备用栈的敌意输入验收（2026-09-21，基于 `0cab79d1`，工作区）
...
### 3.41 当前源码与历史运行证据复核（2026-09-23，`abeba591`）

本节只核对源码和已有报告，未重新构建或运行 QEMU。第 3.40 节记录的九预设 CTest **43/43**、16 CPU/Sv39/GICv3 同镜像补验，是当时的运行证据；不能写成 `abeba591` 的新测试结果。下文的“已实现”只表示当前路径存在，不替代未列出的专项验收。
...
### 3.76 旧 IPC 与生产 capability IPC 分界（2026-09-24）

`2008aa3` 已让生产服务使用进程局部 capability、同步控制 IPC 和共享内存对象，但启动时仍创建旧的 PID/全局服务 ID `IpcManager` 与 `SharedMemoryManager`，并将旧管理器的消息数作为系统 IPC 统计。该计数不包含新服务流量，旧对象也不参与生产服务调用。

`c970c13` 让 native IPC 按需创建对象，生产内核不再初始化或链接旧 IPC 模块，也不再展示旧计数；已无空操作的旧 IPC 启动阶段。旧模块暂留在验证镜像，`ipc_service_lifecycle` 自己持有共享内存 backing 并在退出时恢复测试专用全局指针。九个 workflow 均通过，合计 **53/53 CTest**；宿主 `scripts/tests` 为 **393 passed、1 skipped**。六个保留符号的 Debug/RelWithDebInfo 生产 `moss.elf` 均无旧 `g_shared_memory_manager`/`IpcManager` 符号；Release 镜像已剥离符号，不能据此检查。x64 Debug 验证镜像仍有旧模块符号。

这只关闭生产启动中的双重 IPC 管理器，不等于 ADR-0009～0014 全部完成。传递式 IPC 优先级继承、能力撤销与资源预算、完整服务失败交错，以及内核 POSIX/VFS/驱动的迁移仍需分别验收。

### 3.77 启动机制与旧设备框架分界（2026-09-24，隔离工作树）

此前 DeviceInit 为已启动的 irqchip、timer 和可用 UART 分配 `Device`/`BootDriver`，注册进内核 `DeviceManager` 后通过 probe 检查。实际中断路由、计时器和串口访问始终由 boot/HAL/console 机制负责；登记的设备资源表没有生产消费者，却将旧的匹配和回调模型保留在特权域，并在启动统计中展示为“Registered devices”。

当前 DeviceInit 沿用 boot-owned 中断控制器，验证 `TimerSubsystem` 已初始化，在固件选定 UART 时直接调用生产 `moss.drivers.console::initialize()`；该函数的 IRQ 登记失败仍使启动失败。生产构建仅链接 console 和硬件机制，旧 `moss.drivers` 匹配/回调框架只链接验证镜像，保留已有注册顺序、所有权和失败回滚测试；`console_ring` 改为验证真实启动后的控制台状态及重复初始化。原第 3.41 节的静态绑定和设备数描述是当时的源码事实，不再代表当前生产启动路径。

九个 workflow 全部通过，合计 **53/53 CTest**；宿主 `scripts/tests` 为 **393 passed、1 skipped**。`MOSS_BUILD_TESTS=OFF` 的独立 x64 Debug 构建成功且没有验证 ELF。六个保留符号的 Debug/RelWithDebInfo 生产镜像均无旧 `DeviceManager`/`BootDriver` 符号，x64 Debug 验证镜像仍包含 `DeviceManager`；Release 镜像已剥离符号，不以其符号表作判断。

这仅移除无生产用途的内核驱动注册包装。中断控制器、计时器及早期控制台仍是 ADR-0015 允许的最小启动机制例外；设备资源 capability、用户态枚举/驱动服务、DMA/IOMMU 限制和服务死亡后的资源回收尚未实现。

### 3.78 同步 IPC 的传递式优先级继承（2026-09-24）

`4a4ddea` 与 `ee6f071` 已将 `PendingCall` 持有的优先级捐赠记录接入同步控制 IPC：调用开始记录等待依赖，服务线程收到请求前即绑定捐赠；请求被另一接收者认领时转移，回复、取消、超时、通道关闭或线程销毁时解除。调度器分别计算 RT 优先级和 CFS nice 的有效值，保持基础配置不变，并在有效值变化时重新放置已排队线程。嵌套调用和多个调用者沿依赖链传播；环路按环内基础优先级和外部捐赠重算，避免最后一个外部调用者离开后保留旧提升。

本次让基础 nice 和继承优先级的队列转换在调度转换锁内读取 CPU 归属，避免迁移与重新入队使用不一致的 CPU。进程清理先从运行队列摘除线程；初始监督进程退出时使用同步紧急日志，确保复位前输出诊断。`scheduler/ipc_priority_inheritance` 覆盖就绪线程迁移、嵌套、多捐赠、环路、基础 nice 变更及线程销毁；`users.ipc/nested_roundtrip` 用三个实际进程验证两级同步调用和回复。当前生产 syscall 表没有暴露 RT 基础策略修改入口。

本次九预设 workflow **9/9**、CTest **53/53** 通过；九份已定稿功能报告中的上述两个用例及合入的 `users.ipc/badged_sender` 均通过，ARM64 Release 的监督进程早期失败复位检查也通过。宿主 `scripts/tests` 为 **393 passed、1 skipped**。

这些检查还没有确定性覆盖认领后的取消/超时/reply 竞争、服务线程死亡时回复句柄的交接和端到端倒置时延；CPU 预算、deadline 传播、最大调用深度和死锁策略仍按 ADR-0012 单独决策。现有实现仅是该 ADR 的机制基础，不能据此关闭完整可靠性验收。

### 3.79 已认领 IPC 的取消与服务死亡交错（2026-09-24）

`users.ipc/signal_cancel` 增加已认领请求的交错：服务先收到请求，再向调用方发信号；调用方确认 `EINTR` 和 handler 执行后才放行服务使用一次性 reply，后者必须返回 `EPIPE`。`users.ipc/peer_death` 保持接收端存活，让正在等待的调用只能因持有 reply 的服务退出而得到 `EPIPE`；随后替代服务在同一通道接收并回复下一次调用，验证失败调用的清理没有损坏通道。

信号门控逻辑实现后，九预设 workflow **9/9**、CTest **53/53** 通过；随后为新增单行条件补齐花括号，x64 Debug **6/6 CTest** 再通过。九份功能报告中的上述两个用例均通过；宿主 `scripts/tests` 为 **378 passed、1 skipped**。

这些回归关闭了上述两个具体交错的验证缺口。已认领请求的 deadline 与并发 reply 竞争、回复句柄转移、端到端优先级倒置时延，以及 ADR-0012 的 CPU 预算、deadline 传播和死锁策略仍需后续验收。

### 3.80 已认领 IPC 的 deadline 与迟到 reply（2026-09-24）

`users.ipc/deadline` 原来只让无人接收的请求到期，不能证明请求已被服务认领后仍可到期并使迟到 reply 失效。新增交错由子进程发起带绝对 deadline 的调用，父进程先接收并持有一次性 reply；子进程返回 `ETIMEDOUT` 后经管道通知父进程，父进程才调用 reply，并要求它同样返回 `ETIMEDOUT`。管道顺序确保迟到回复发生在调用方已观察到超时之后，不靠固定睡眠决定竞争结果。

九预设 workflow **9/9**、CTest **53/53** 通过；九份已定稿功能报告中的 `users.ipc/deadline` 均通过。

这只覆盖已认领请求的超时与迟到回复。reply 与取消同时发生时的唯一胜者、回复句柄交接、端到端优先级倒置时延，以及 CPU 预算、deadline 传播和死锁策略仍未验收。

### 3.81 同步 reply 与信号取消的胜者一致性（2026-09-24）

`users.ipc/signal_cancel` 增加两个已认领请求的交错：服务先完成 reply 再发信号时，调用方仍必须得到回复；服务先发信号并立即 reply 时，允许任一方赢，但调用方得到回复仅当服务的 reply 成功，调用方得到 `EINTR` 仅当服务的 reply 返回 `EPIPE`。两种路径都要求信号 handler 执行、服务正常退出；结果经管道传回调用方，读取和等待子进程时重试被该信号打断的系统调用。

九预设 workflow **9/9**、CTest **53/53** 通过；九份已定稿功能报告中的该用例均通过。x64 Debug 的 `users.ipc` 另重复运行五次，均通过。

这验证一次性回复与取消结果的一致性，没有为跨线程或跨进程转交 reply 句柄建立新权限，也没有量化优先级倒置时延。

### 3.82 工厂 capability 授权的原生执行域构造（2026-09-24，隔离工作树）

`SYS_DOMAIN_FACTORY` 只允许内核指定的初始启动域铸造工厂 capability；普通 fork 子进程不会继承这一铸造资格。装载者必须持有 `DOMAIN_SPAWN` 权限才可调用 `SYS_DOMAIN_SPAWN`，工厂句柄可经显式 capability 传递交给后续 Loader Service。`SYS_DOMAIN_LAYOUT` 给出当前架构/MMU 的用户地址、栈和保留区布局，避免 RV64 的 Sv39/Sv48 运行时差异被用户态硬编码。

`SYS_DOMAIN_SPAWN` 从调用方同一个已持有的地址空间版本读取描述符、页面字节和选定句柄；内核拒绝越界、重叠、保留区及 W+X 映射，建立私有页和初始栈，只在全部准备成功后发布可运行域。失败时未发布的地址空间与已建进程回滚；成功返回域 capability，内部 PID 仅用于诊断且不建立 POSIX 父子关系。ARM64 的普通用户只读/可写页补上执行禁止，用户 RX 页补上特权执行禁止；首次用户调度同步本 CPU 指令缓存。

`users.ipc/domain_spawn` 验证无工厂权限、收窄权限、fork 子进程私自铸造、W+X、非代码入口和不可读源缓冲区均被拒绝，并让新域从提交的代码页运行、读取初始栈后以指定状态退出；`waitpid` 不把诊断 PID 当作子进程。九预设 workflow **9/9**、CTest **53/53** 通过，九份已定稿功能报告中的新增用例均通过。

这只闭合原生域构造机制。单次镜像最多 4096 个 4 KiB 页面，初始栈受当前 32 KiB 预算限制；普通 ELF 仍由内核 `execve` 解析和加载，用户态 Loader Service、授权文件/Memory Object 输入、POSIX `exec` 原子身份替换及可执行内容批准与启动信任链仍待实现（ADR-0024～0026）。本节原先允许调用者直接提供 RX 页字节；3.83 将其收紧为必须引用已批准的不可变代码版本。

### 3.83 不可变代码版本与独立执行授权（2026-09-24，隔离工作树）

`SYS_CODE_SNAPSHOT` 将调用者的一页字节复制为内核持有的不可变代码版本；`SYS_CODE_READ` 只允许持有读权限的服务检查这一确切版本。只有初始启动域可铸造独立的代码批准 authority，持有 `CODE_APPROVE` 的策略服务才能通过 `SYS_CODE_APPROVE` 为某一版本派生 `CODE_EXEC` 句柄。代码版本和批准句柄均可按 capability 权限显式传递，不能仅凭域工厂或普通 Memory Object 的可写权得到执行权。

`SYS_DOMAIN_SPAWN` 现在拒绝直接从可写调用者缓冲区建立 RX 页：可执行页面必须引用持有 `CODE_EXEC` 的代码版本，内核把获批的完整页复制到新域的私有 RX 帧。原始缓冲区后续修改或释放不会改变获批版本及新域代码，也不会形成跨域可写/可执行物理别名。现有 IPC 验证覆盖未批准版本、无批准权限、收窄 authority、源缓冲区改写后读取快照和新域执行，以及工厂 capability 经 IPC 显式委托。初始启动域的普通 fork 子进程不能自行铸造工厂或代码批准 authority。

九预设 workflow **9/9**、CTest **53/53** 通过；九份最新已定稿 `users.ipc` 报告的 `domain_spawn` 用例均通过。主机 `scripts/tests` 为 **378 通过、1 跳过**。针对修改行的 `clang-analyzer` 与 `clang-format`、Ruff 均通过；全量 `lint.py` 仍被既有展开排除列表触发的 `E2BIG` 阻断，全仓 `format --check` 仍报告本次修改范围以外的旧格式差异。

这是 ADR-0026 的内核机制切片，不是产品代码审核策略：用户态 Code Authority Service、授权文件到代码版本的身份协议、服务重启与批准撤销、Pager/设备共享别名、JIT 生命周期及 ADR-0027 启动信任链仍待实现。本节先采用一页一个代码版本句柄；3.84 将其扩展为多页范围。

### 3.84 有界多页代码版本（2026-09-24）

`SYS_CODE_SNAPSHOT_RANGE` 一次复制连续的调用者页面，生成一个内核持有的不可变代码版本；`SYS_CODE_READ_RANGE` 让持有读权限的策略服务按页审查相同的版本。原有 `SYS_CODE_SNAPSHOT` 和 `SYS_CODE_READ` 仍分别对应单页快照和第 0 页读取。整个范围经一次 `SYS_CODE_APPROVE` 授权，可执行页用 `moss_domain_page.code_page_index` 指定该版本中的页面；内核拒绝越界索引，普通数据页不得指定版本或索引。描述符增加索引字段，使用旧结构布局的二进制需重新编译。

每个版本最多 4096 页（16 MiB），与单次执行域构造上限一致。复制失败时未发布句柄，已复制页面随版本对象销毁，当前页面直接释放。`users.ipc/domain_spawn` 使用 65 页的一个版本（超过 64 个 capability 槽位），在快照后清空源缓冲区，读取完整范围，再以最后一页启动新域；同时验证旧单页接口、部分源不可读、超限请求和越界索引。

九预设 workflow **9/9**、CTest **53/53** 通过；九份最新已定稿 `users.ipc` 报告中的 `domain_spawn` 用例均通过。

这是用户态 Loader Service 装载较大文本段的内核前提。Loader Service、真实 ELF/授权文件输入、按域资源配额、Code Authority Service、批准撤销和启动信任链仍待实现；有界单次请求不能代替资源配额。

### 3.85 代码审批可核对完整版本长度（2026-09-24）

一次 `SYS_CODE_APPROVE` 授权整个不可变范围，因此审批服务不能仅凭请求者报告的页数决定检查多少页。新增 `SYS_CODE_PAGE_COUNT` 返回代码版本对象在内核记录的完整页数，要求 `MAP_READ`；仅有 `CODE_EXEC` 的执行句柄不能借此读取审查元数据。`users.ipc/domain_spawn` 验证单页版本返回 1、65 页版本返回 65，并验证执行句柄查询遭拒。九预设 workflow **9/9**、CTest **53/53** 通过，九份最新已定稿 `users.ipc` 报告中的该用例均通过。

这只提供可核对的版本边界。实际审批策略、文件身份和服务协议仍待定义；调用方报告的长度本身不构成批准依据。

### 3.86 同步 IPC 优先级倒置时延验收（2026-09-24）

`users.ipc/priority_latency` 在同一个 CPU 上运行高优先级调用者、低优先级服务和八个中优先级的持续计算进程。服务收到请求后先完成一段需要多次调度机会的计算，再用一次性 reply 回复；调用以一秒内完成为本回归夹具的界限。负载维持两秒，以确保没有捐赠时调用先到截止时间。该界限用于检测优先级倒置回归，不是对外服务时延承诺。

x64 Debug 的反向对照临时禁用 `CfsScheduler::bind_ipc_server` 捐赠时，调用在一秒截止前未完成；恢复原有调度代码后，同一夹具约 17 毫秒完成。临时改动未保留在提交中。最终用例在 x64、ARM64、RISC-V Debug 的 QEMU `users.ipc` 定向运行均通过；随后九预设 workflow **9/9**、CTest **53/53** 通过，九份最新已定稿 `users.ipc` 报告中的 `priority_latency` 均通过。

这证明当前同步调用路径在该绑核竞争场景下把调用者优先级传给服务；不证明所有调度负载下的尾延迟或实时截止保证。回复句柄交接、CPU 预算、deadline 传播及死锁策略仍待实现和验收。

### 3.87 原生域代码批准实例与撤销（2026-09-24）

此前 `SYS_CODE_APPROVE` 只给同一个不可变 `CodeVersion` 增加 `CODE_EXEC` 句柄；复制或转交该句柄后，内核没有独立批准实例可供撤销，同内容重新批准也无法区分新旧授权。现在每次批准生成独立的 `CodeApproval` 对象，持有原始不可变版本和颁发它的 `CodeAuthority` 作用域。`SYS_CODE_REVOKE=164` 需要该作用域的 `CODE_REVOKE` 权限及目标实例的 `CODE_IDENTIFY` 权限；两者均可从启动域铸造的完整 authority/approval 句柄分别削减和委托，只有识别权限不能执行，只有撤销权限不能批准或执行。新权限占位为 bit 13/14；原有代码读取、页数查询及未批准版本执行的拒绝结果仍为 `EACCES`。

撤销设置实例状态，不关闭复制或转交出去的句柄；这些句柄仍指向旧实例，不能再为 `SYS_DOMAIN_SPAWN` 授权。用同一不可变版本重新批准会生成新的实例，旧句柄不会复活。新域构造先把代码复制到私有页，在全部可能失败的准备结束后，持有所有使用过的批准实例引用并在与撤销共用的锁下完成最后准入检查；检查通过是这一批新域映射的准入提交点。之后撤销不回收已建立的可执行映射，也不终止域。新用例验证独立作用域、批准/撤销/识别权限削减、复制及 IPC 转交的旧句柄、同内容重新批准、撤销前已创建域的继续执行，以及新实例再次撤销。x64 Debug 临时让撤销不改变状态时该用例按预期失败；生产实现已恢复。三架构 Debug 的 `users.ipc/code_revocation` 定向运行及九预设最终 `users.ipc` 报告均通过。完整构建矩阵并发运行时，x64 Release 与 RelWithDebInfo 的 `vfs` workload 曾超时；矩阵结束后串行重跑这两个 CTest 预设均通过，最终九预设合计 **53/53 CTest**。

这一阶段只管原生 `SYS_DOMAIN_SPAWN` 的执行准入。普通内核 `exec`、fork 后可执行映射、权限升级及未来 pager 路径仍未迁到批准机制；Code Authority Service、supervisor 独立保留撤销作用域、服务失败恢复、确定性并发提交/撤销交错和终止权限隔离的完整验收也未完成。因此 ADR-0028～0032 仍保持打开。

### 3.88 代码批准服务进程退出后的权能存续（2026-09-24）

验证用例让独立 fork 子进程模拟一次代码审批服务 incarnation：父进程仅委托批准权而不委托撤销权，通过 IPC 传入不可变版本的读取权，子进程批准并返回绑定该实例的执行/识别句柄，随后退出。父进程先关闭完整 authority，只保留独立削减的批准委托权和撤销权；在发行者退出后，旧批准仍可建立并执行原生域，说明内核的已提交实例不依赖服务继续运行。父进程仍能撤销该实例，并阻止旧句柄再准入。未得到新委托的替代子进程不能铸造 authority 或批准代码；显式重新委托后可发行新实例，旧实例仍保持撤销状态；仅有撤销权也不能批准。九预设串行构建和 CTest **53/53** 通过，九份最终 `users.ipc/code_service_survival` 报告均通过。

这是 ADR-0031/0032 的权能生命周期和显式交接机制验收，不是实际 Code Authority Service 或产品签名策略。服务请求在提交边界的失败竞态、真实 supervisor 重启协议、正常 `exec`/pager 的执行准入及启动信任链仍未实现或验收；ADR-0026～0033 的总项继续打开。

### 3.89 独立代码审批服务与 supervisor 交接（2026-09-24）

该切片的生产 initramfs 包含独立的 `code-authority-service.elf`。初始 supervisor 铸造单个 `CodeAuthority` 作用域，分出可用于显式重启委托的 `CODE_APPROVE|DUPLICATE` 句柄和独立 `CODE_REVOKE` 句柄后关闭完整句柄。服务由 `SYS_FORK_DOMAIN_SELECT` 启动，只得到 `CODE_APPROVE` 和私有 IPC 接收端；在本节时点，supervisor 仅保留不可复制、不可转交的 `SEND` 请求端。策略是“supervisor 明确提交的不可变版本可批准”：服务检查传入的 `MAP_READ` 版本句柄和内核报告的页数，批准后返回独立实例。本节时点没有把请求端委托给 shell、普通进程或 Loader Service，也没有产品签名策略；3.90 对受信任 Loader 增加了限定委托。

启动时 supervisor 对自身文本页做一次快照并通过真实 IPC 请求批准，保留返回的实例作为探针。服务进程退出后，supervisor 用独立的撤销句柄撤销该存续实例，关闭旧端点，再显式把批准权交给新服务，重建端点并再次请求批准。管理 shell 随后刷新它所持有的服务域终止句柄；文件、命名空间和进程服务保持原 incarnation。生产启动回归用 `moss-domain.elf terminate code` 触发这一过程，核对新服务 PID、shell 恢复及已有文件内容继续可读。合并前主线的三架构九预设串行 workflow **53/53 CTest** 全部通过，九份生产启动结果均完成 92 步并确认代码服务恢复。与 Process/Pipe/Console 服务合并后，本分支 x64 Debug 全套 6/6、六配置应用和生产启动 12/12、用户态构建 8/8，以及关闭测试的 x64 生产镜像 146 步通过；其余合并态九预设整套检查尚未重跑。

这只建立受限的用户态审批入口和真实 supervisor 失败交接。supervisor 为重启保留的批准委托本身仍可直接调用批准 syscall，因此尚无防止 supervisor 越过服务的更强隔离。服务镜像仍由现有内核 ELF 路径装载，尚无已认证启动链；普通 `exec`、fork、pager、权限升级、产品代码身份/签名策略、并发撤销交错和真机交接仍待实现，ADR-0026～0033 总项保持打开。

### 3.90 用户态 Loader Service 的首条原生程序路径（2026-09-24）
...
### 3.91 Reply capability 跨进程交接（2026-09-24）

此前接收端只得到 `SEND` Reply 句柄，而消息转交要求 `TRANSFER|DUPLICATE`，所以服务无法把回复权交给另一线程或进程。现在收到的 Reply 带 `SEND|TRANSFER`、不带 `DUPLICATE`；它可随同步 IPC 的请求或响应移动到另一进程，并可进一步削减为仅 `SEND`。能力表在同一把锁下把源句柄转换为消息 escrow：源句柄立即失效，escrow 在目标句柄安装前维持对象生命；普通 capability 仍按既有复制语义转交。丢弃未交付消息或关闭最后一个 Reply 句柄时，仍在等待的原调用方得到 `EPIPE`。Reply 不能自附于其响应，以免调用持有自己的 Reply 形成引用环；回复 syscall 在提交前原子消费表中句柄，使并发转交与回复只有一个持有者胜出。

请求交接在选择接收线程、响应交接在提交回复时，将原调用者的优先级捐赠重绑给新持有者，均发生在唤醒低优先级线程之前。最后一个通道接收句柄关闭会拒绝新请求并取消尚未交付的请求；已交付调用仍由 Reply 持有者、调用方取消或原 deadline 决定终态。`users.ipc/reply_handoff` 验证请求路径跨进程移动、旧句柄失权、不可复制/继承及一次性回复；`reply_handoff_drop` 验证受托者丢弃句柄时原调用方得到 `EPIPE`。`reply_handoff_latency` 验证原服务经响应交给低优先级工作者并退出后，工作者在同核八个中优先级负载下仍于一秒调用截止前回复；负载持续两秒。x64 Debug 临时关闭响应提交时的捐赠重绑后，该新增时延用例按预期超时，原 `priority_latency` 仍通过；临时代码未保留。

最终三架构九预设串行 workflow **9/9**、CTest **53/53** 通过，九份最终 `users.ipc` 报告中的三个新增用例均通过；九份生产启动各完成 **109 步**。宿主 `scripts/tests` **378 passed、1 skipped**，Ruff、改动范围格式和 `git diff --check` 通过。这是 ADR-0011/0012/0023 的回复权移动与优先级机制验收；CPU 预算、deadline 向下游传播、最大调用链深度、死锁策略及硬件时延保证仍未完成。此前各节标出的“交接待做”属于当时状态，以本节及当前清单为准。

### 3.92 Reply 转交失败时的权能回滚（2026-09-24）

3.91 的成功交接已保证 Reply 只能移动一次，但请求携带 Reply 时，原句柄在目标通道确认入队之前就被撤销。若端点已关闭、队列满、调用期限已过、在途调用分配失败或线程无法开始等待，临时 escrow 随失败路径销毁，原调用方收到 `EPIPE`，服务却未成功转交回复权。回复携带另一个 Reply 时也有同类问题：当前回复可能输给取消或期限到期，附件却已被移动。
...
### 3.94 clang-tidy 排除过滤器的参数长度（2026-09-24）

提交前质量门禁在 ARM64 Debug 构建成功后、clang-tidy 启动前报 `Argument list too long`。`lint.toml` 的两条 vendored 排除规则匹配 3363 个已跟踪文件；`lint.py` 将逐文件转义后的正则拼成约 181 KiB 的单个 `--exclude-header-filter` 参数，超过 Linux 单个参数的长度限制。此前的错误是工具无法启动，不是源码诊断。

现在仍按原有 Git 排除语义展开文件集合并构造相同正则，但只把该选项写入构建目录中的 clang-tidy response file，子进程命令行仅包含该文件路径。文件在启动并发检查前写好，避免任务间重写。新增宿主用例构造超过 128 KiB 的过滤器并核对参数文件内容；`scripts/tests/test_lint.py` **47/47** 通过。真实 `lint.py --check --preset arm64-debug src/intrinsics/src/intrinsics-bitops.cppm` 完成构建并成功检查该翻译单元；Ruff、改动文件格式及 `git diff --check` 通过。此修复只恢复 lint 工具运行，不替现有源码的大括号诊断或未改文件的全仓格式差异背书；MOSS-032 总项继续开放。

### 3.95 Loader 原生域启动参数（2026-09-24）
...
这次整合后的九预设 workflow 再次 **53/53 CTest** 通过，九份生产启动报告均为 `passed`、各 **165 步**；原始日志与 JSON 保存在 `build/results/merge-master-libc/`。相关宿主测试 **20/20** 通过。该阶段仍未合入随后提交的 Loader 入站封存门禁；以上数字只覆盖本节这一阶段的源码。

### 3.99 Loader 入站文件封存门禁（2026-09-24）

3.97 在 supervisor 写入启动镜像后封存文件，但 Loader 的 `RUN` 入口仍只要求一个 `SEND` 句柄。如果后续增加其他调用方，单靠 supervisor 约定无法保证 Loader 实际读取的来源在多页读取期间保持不变。现在 Loader 在校验启动参数后、第一次文件读取前，对收到的对象发送幂等 `SEAL`，并检查精确的成功状态和空权能响应；失败时返回 `NO_IMAGE`，不会申请代码批准或创建执行域。保留 supervisor 在交接前的封存及写入、缩容负向检查，使交接和读取两端都能验证同一对象的状态。

每次启动 Loader Service 时，supervisor 从 File Service 打开公开具名的 `/scratch`，通过真实 `RUN` 请求交给 Loader，要求返回 `NO_IMAGE` 且无域句柄。这个负向路径证明 Loader 在读取或解释文件内容前执行封存门禁；File Service 对具名文件拒绝 `SEAL`，避免普通读取者借此阻止写入。现有三份不具名镜像仍继续通过装载或坏镜像拒绝检查。此切片使受限 Loader 的入站文件身份不再依赖单一调用方；普通 POSIX `execve`、动态链接、持久文件身份和恶意文件服务仍未迁移或验收。

验收：三架构 Debug、Release、RelWithDebInfo 九预设 **53/53 CTest**，11 份生产启动报告均为 `passed`、各完成 **113 步**（ARM64 Debug 三次，其余各一次）；只补分支大括号后，最终源码又通过 x64 Debug **6/6 CTest** 和 **113 步**生产启动。宿主 `scripts/tests` **380 passed、1 skipped**；改动 C 文件与协议头的 clang-format、Ruff 和 `git diff --check` 通过。定向 clang-tidy 仍因仓库既有单句分支的大括号规则失败，关闭该规则后两个改动翻译单元无其他诊断。验证范围为 QEMU 与宿主，没有覆盖真实硬件、持久文件服务或恶意服务实现。

本工作树随后将入站门禁与 3.98 的启动归档、静态 libc、`APPEND` 封存拒绝及受管子进程交接合并。常规 Loader 由 supervisor 传入当前 File Service 根句柄，每次启动都以具名 `/scratch` 发出真实 `RUN`，确认 Loader 自行封存失败时返回 `NO_IMAGE`；已经由 supervisor 封存的三份私有镜像仍可通过幂等 `SEAL` 装载或被识别为坏镜像。合并后的九预设 workflow **53/53 CTest**，九份真实 QEMU 生产启动报告均为 `passed`、各 **165 步**；宿主 `scripts/tests` **381 passed、1 skipped**，Ruff、改动行 clang-format、定向 clang-tidy（关闭既有单句大括号规则）及差异检查通过。原始日志和生产 JSON 见 `build/results/merge-master-loader-gate/`。这些结果不证明普通 POSIX `execve`、持久文件或恶意服务的镜像身份已解决。

### 3.100 同步 IPC 嵌套调用的 deadline 继承（2026-09-24）

此前每次 `SYS_IPC_CALL` 只使用本次调用显式传入的绝对 deadline；服务线程接到有截止时间的请求后，若对下游服务发起无截止时间或更晚截止时间的嵌套调用，外层调用过期后仍可能被下游阻住。现在同步等待依赖保存其绝对 deadline，并在服务线程建立下一次调用时，于 IPC 依赖锁下取显式 deadline 与当前活跃调用者 deadline 的最早非零值。该值写入新请求、一次性 Reply 交接所携带的依赖和定时器；若建立调用时已过期，则在入队前撤销等待依赖并返回 `ETIMEDOUT`。

截止时间在嵌套调用开始时取快照。之后才交给该线程的新请求不追溯缩短已在途的下游调用；一个线程同时持有多个调用者 Reply 时，取当前最早截止时间作保守界限。调度器回归以合成绝对时间检查显式较早值、多调用者最早值、两级传递与依赖结束清理；真实三进程 IPC 中，外层两秒截止、内层传零、后端三秒后回复，验证外层和内层均超时且迟到 Reply 被拒绝。CPU 预算、在途调用的动态 deadline 收紧、最大调用链深度及调用环策略仍属于未完成的 ADR-0012/0023 项。

验收：三架构 Debug、Release、RelWithDebInfo 九预设 **53/53 CTest**，11 份生产启动报告均为 `passed`、各完成 **113 步**；新增 `users.ipc/deadline_propagation` 在九预设全部通过。补齐新增分支的大括号后，最终源码又通过 x64 Debug **6/6 CTest**、**113 步**生产启动，新增用例通过。宿主 `scripts/tests` **380 passed、1 skipped**；改动 C/C++ 文件的 clang-format、Ruff 与 `git diff --check` 通过。全文件 clang-tidy 仍被仓库既存诊断阻断，按本次新增行过滤后零诊断。运行验收限于 QEMU 与宿主；在途调用的动态 deadline 收紧、真实硬件时钟与恶意服务实现尚未覆盖。

本工作树将上述 deadline 继承与原生文件、Loader、Process Service 路径合并后，重新运行三架构 Debug、Release、RelWithDebInfo 九预设，**53/53 CTest** 全部通过；九份最新生产启动报告均为 `passed`、各完成 **165 步**，新增 `users.ipc/deadline_propagation` 在九预设全部通过。宿主 `scripts/tests` **381 passed、1 skipped**，Ruff 与差异检查通过。逐预设 CTest 日志、功能报告、生产报告和汇总保存在 `build/results/merge-master-ipc/`。这些结果不覆盖真实硬件、恶意服务或完整 POSIX 路径。

### 3.101 具名文件的不可变装载快照（2026-09-24）

此前 File Service 只允许不具名对象 `SEAL`；Loader 因此必须拒绝具名文件，无法从可继续写入的源文件取得稳定镜像。现在只有持有文件服务根发送端的调用者可按名称请求 `SNAPSHOT`；普通具名文件 `SEND` 句柄不能凭单个对象反复创建副本。File Service 在单接收循环内复制当前长度和字节，创建新的不具名、已封存对象并转交发送端。原文件保持可写，快照的 `WRITE`、`APPEND` 和 `RESIZE` 被拒绝。复制计入现有 16 MiB 内容总额和 16 个易失对象上限；启动归档文件单独计数，若回复交接失败，刚创建的快照从对象表和配额中回滚。Namespace Service 的 `CREATE` 仍能申请具名对象，完整客户端资源计费并未实现。
...
### 3.102 原生文件描述符的内容配额错误（2026-09-24）
...
| MOSS-004 | P0 | 堆与页表/PFA 所有权重叠 | 已关闭（`e7fe8c91`）；当前布局、活动页表树/早期表池及元数据哨兵、耗尽、坏布局启动拒绝专项验收通过，见 3.7。 |
...
| MOSS-006 | P0 | 容器与使用者的所有权 | 部分修复；伪 RCU 已替换为 LockedList/LockedHashMap，持有读者与双 CPU 交错有实测；生产启动已退出旧 DeviceManager/BootDriver，旧匹配框架仅留验证（3.77）；IRQ 注销等待回调（3.52），IPI 部分注册失败会回滚（3.53），旧 IPC 服务/通道发布与注销串行化（3.54）仅保留测试，生产不再链接该模块（3.76）；实际硬件回调、隔离驱动资源解绑及 native IPC 进程清理/在途消息仍待验收。 |
...
| MOSS-013 | P1 | 对齐 / buddy / 释放契约 | 已关闭（`e7fe8c91`）；heap/PFA 对齐、释放归属、保留洞分段/耗尽专项验收通过；页引用并发不在此结论内，见 3.4～3.5。 |
...
**2026-09-06 更新：部分修复，工作区。** `c895c876` 的指针发布修复见 3.8；当前已删除 RcuList/RcuHashMap、读锁和回调池，使用 IRQ-safe LockedList/LockedHashMap，查找返回值副本或 shared_ptr。VMA、进程、WaitQueue、IRQ、驱动和 IPC 调用者已迁移；析构与可能重入的快照回调在解锁后执行，3.9 保留持有读者的红绿对照及真实双 CPU 交错证据。下列旧 RCU 源码是原始审计事实，不再代表当前实现；所有使用者的复合生命周期尚未验收，本项和 A4 仍不关闭。
...
**2026-09-14 更新：** `3461c749` 已将三 ISA 改为原生帧单指针入口并完成 3.19 的寄存器/信号正常路径验收；下面的第八参数/ARM64 共用数组描述仅保留为历史根因。
...
**2026-09-06 更新：已关闭，修复提交 `e7fe8c91`。** `PageFrameAllocator::initialize_free_lists` 循环选择符合物理地址对齐、剩余长度和保留区约束的 order。heap 修复及五项用例见 3.4；PFA 原分配头/order、所有页的引用/标志、完整范围检查及两项真实套件见 3.5。六配置 CTest 与 ARM64/RV64 固件边界检查已通过本项所列对齐、错误释放和保留洞后的分段/耗尽验收。此结论不覆盖页引用 API 自身的并发/下溢或完整 COW 生命周期；那些仍需 008～010/028 验收。
...
**2026-09-06 更新：部分实现。** `f7768373` 的 `sys_fork` 已提取 x86 PC/SP、RV64/x86 GP 寄存器；首次运行先在新内核栈保存完整 CpuContext，再经架构 trampoline 返回用户态。三架构 `validation.c` 的一次 fork/exec/exit/wait 已通过。尚需 fork 不立即 exec 的快照验证、VM 元数据/凭据/信号继承、FP/SIMD 状态与资源失败回滚；共用 syscall 仍手写架构槽位。
...
**2026-09-20 更新：已关闭。** UnifiedMemoryManager 的未实现查询/高级操作、BuddyAllocatorV2 的占位初始化/压缩/watermark/压力/碎片/统计，以及旧 IPC 模块中尚无真实进程映射所有权的 map/unmap/cleanup 均返回明确的 `NotSupported` 且保持状态不变。旧共享区 create/destroy/stats/sync 使用实际分配对象；假 LOW/100%/零统计和固定映射地址已删除。初始化采用四态原子发布，公开原始 singleton 借用已移除；红绿证据及九配置验收见 3.28～3.29。新 capability 内存对象的映射路径是后续独立实现，不应混同于旧 IPC 接口；未实现能力仍是后续功能，不因本项关闭而被宣称可用。
...
第 3.41 节记录的启动静态绑定已由 3.77 的直接启动机制检查取代：生产不再创建 DeviceManager/BootDriver，也不再打印无意义的注册设备数。UART 硬件操作仍由 HAL 承担；这不等于动态设备管理、热插拔或通用 DMA 已可用。启动阶段打印成功、固定统计和末尾 ALL TESTS PASSED 同样需要与真实条件绑定。
...
- [x] A1a：建立生产内核功能套件、框架失败/panic/timeout 自检、串口协议和宿主报告；六配置已有通过记录（028，`f7768373` / `83f5fa94`）。
...
- [x] A2a：三个 linker 预留 8 MiB heap，起始 arena/扩容不得越过 heap_end；加入并通过 heap_bounds（004，`f7768373`）。
- [x] A2b：PFA 选择真实 RAM bank，metadata 和自由块跳过 initrd/固件保留区（005，`f7768373` / `83f5fa94`）；这里只标实现，不标穷尽验收。
...
- [ ] A4：替换错误的自动退休容器；先用明确锁与拥有关系闭合生命周期（006）。
  - [x] 指针发布不自动析构仍通过 next 可达的旧节点（`c895c876`，3.8）。
...
- [x] B3a：RV64/x86 首次用户返回与基本 fork 用户 GP 现场恢复已补，三架构真实 users 套件通过（007、014，`f7768373`）。
- [ ] B3b：三个 ISA 有效 TrapFrame 传参、布局断言、完整用户现场、信号桩及安全返回仍待统一（007）。
  - [x] 单一原生帧入口、全部布局断言、GP/六参数/条件码往返、fork 和 native sigreturn，九配置通过（`3461c749`，3.19）。
...
- [ ] B4：信号帧复制与返回状态净化，修复结果/handler 参数写回顺序（003、020 的入口部分）。
  - [x] 结果先写回、再保存信号现场与设置 handler 参数；三 ISA 的真实 GP/返回值信号往返通过（`3461c749`，3.19）。
...
- [x] C1a：正常 PVH 模块发现、用户态基本闭环及复用生产启动的验证镜像已接通（027、028，`f7768373` / `83f5fa94`）。
...
- [x] D3a：删除 C++ inline 换栈，复用已有 context_switch 返回活跃 bootstrap 调度上下文（023，`f7768373`）。
...
- [x] E3a：x86 TSC/LAPIC 独立 PIT 校准，RV64 DTB timebase + SBI TIME；无 Sstc 配置已有验收（029，`83f5fa94`）。
