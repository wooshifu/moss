...
> 状态复核：2026-09-14，提交基线：`0e88344`。第 3.4～3.19 节保留此前实测及当时状态；最新 uaccess 进展见第 3.20 节。本文与 [todo.md](todo.md) 同步。
...
1. **隔离缺陷尚未全部闭合。** 已修复内核映射 USER，并收紧三架构最终内核 W^X；syscall 0 原始 UART 指针旁路已删除，用户地址域/VMA 准入与跨 VMA 权限校验已补。`0e88344` 改为原生 TrapFrame、按 ISA 净化信号返回状态；第 3.20 节补共享用户复制的异常 fixup。VFS/信号复制迁移、恶意信号帧和并发 VM 生命周期仍未完成验收。
...
3. **跨架构入口已统一，完整进程语义仍待补。** 三 ISA 用单一原生帧指针传递 syscall，fork 不再猜内核栈偏移，信号返回桩由各 ISA 汇编产生。GP/条件码和基本信号往返通过，不代表全部 FP/TLS 继承、CPU-bound 信号、nanosleep、VM 并发事务已经可靠。
4. **旧停滞记录应保留，不能直接当作当前复现。** 退出已改为经 `context_switch` 返回活跃 bootstrap 栈，不再在 C++ 帧中直接修改 SP。第 28 次停滞的原版本对照、1,000 次生命周期与资源稳定性尚无完整关闭证据。
5. **测试已从基础语言检查升级为真实内核检查，覆盖仍有边界。** 三架构已有功能、框架自检和 Release 基准证据；默认套件包含 COW 权限、原生帧及用户输出复制的缺页 OOM 回归，但未完成恶意信号帧、全部 uaccess 故障、长循环或确定性调度竞争验收。
...
| MOSS-002 | P0 | 用户域 / uaccess / 调试旁路 | 部分修复（3.13、3.20）；原始 puts 旁路已删除，统一准入与共享复制异常 fixup 已补，输出缺页 OOM 返回 EFAULT 有三架构实测；VFS/信号迁移、全部故障覆盖及页/VM 生命周期未完成。 |
| MOSS-003 | P0 | 信号帧与特权状态恢复 | 部分修复（3.19）；原生帧、PC/SP 用户域与状态白名单已补，普通信号往返通过；fault-safe copy、恶意帧/altstack 全验收未完成。 |
...
**当前状态：** 第 3.13 节已补地址域、VMA 准入/跨段权限检查和字符串长度错误，删除 syscall 0 先 puts 后分发的旁路；第 3.20 节补共享复制异常 fixup 和输出缺页 OOM 回归。VFS/信号迁移、全部故障覆盖与 VM 生命周期未完成，整项不关闭。
...
**2026-09-14 更新：** 原生帧、地址/状态净化及基本信号返回已实现（3.19）；下述原始风险不再等于当前仍能直接恢复任意特权状态。安全复制和恶意/备用帧完整验收仍待完成。
...
- [ ] B2：统一用户地址域与可恢复 uaccess，移除 syscall 0 调试旁路（002）。
  ...
  - [ ] 页/VM 生命周期、VFS/信号用户缓冲区迁移及其余输入/COW/跨页故障专项验收。
...
- [ ] B4：信号帧复制与返回状态净化，修复结果/handler 参数写回顺序（003、020 的入口部分）。
  ...
  - [ ] 可恢复复制、恶意/嵌套/备用帧与 IRQ CPU-bound 信号专项。
