...
| MOSS-006 | P0 | 容器与使用者的所有权 | 部分修复；伪 RCU 已替换为 LockedList/LockedHashMap，持有读者与双 CPU 交错有实测；启动设备已通过 DeviceManager/BootDriver 静态注册并激活；IRQ 注销等待已进入回调（3.52），实际硬件回调、动态解绑和 IPC 复合生命周期仍待验收。 |
...
**2026-09-23 更新：部分验收。** IRQ 描述符新增回调准入/注销同步：注销返回后，之前已取得描述符的回调不再持有注册者借出的 `context`；双 CPU 确定性交错和去同步红例见 3.52。实际硬件 IRQ 回调、动态驱动解绑及 IPC 创建/销毁的复合生命周期尚未闭合。
