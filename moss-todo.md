...
> 源码复核基线：2026-09-23，`604fe85`（当次未重跑 QEMU）。第 3.4～3.20 节保留此前实测及当时状态；后续实现与运行记录见第 3.21～3.72 节，源码与历史证据的对应关系见第 3.41 节。本轮九预设运行见第 3.59～3.72 节。本文与 [todo.md](todo.md) 同步。
...
5. **测试已从基础语言检查升级为真实内核检查，覆盖仍有边界。** 本轮九预设 CTest 为 44/44（3.72），包含 Release 的三个 benchmark、ARM64 Debug console 登记探针，以及 ARM64/x64 的真实 IRQ/等待登记交错。默认用户套件包含 COW/OOM、恶意信号帧、部分用户复制故障、双 console 读者、管道与 nanosleep 信号中断、限定 SA_RESTART 重试、基本 STOP/CONT、SA_NOCLDSTOP、SA_NOCLDWAIT/显式 SIG_IGN 自动回收、进程组 wait 筛选、waitpid 停止/继续与信号致死状态报告及 1,000 次生命周期；这不证明全部 uaccess 故障、确定性调度竞争或真机可靠性。
...
此项覆盖稳定进程组成员关系下的 wait 筛选和等待唤醒；当时组成员在等待期间变化的交错、`SA_NOCLDWAIT`、信号 action 的并发更新与完整 job control 仍保留在 MOSS-020。`SA_NOCLDWAIT` 的后续修复见 3.72。
...
| MOSS-020 | P1 | 信号投递 / STOP/CONT/SIGCHLD | 部分修复；结果/handler 参数写回、SIGCHLD、pipe/console 中断与部分传输、wait/nanosleep EINTR、限定 SA_RESTART 重试及 CPU-bound IRQ 投递已验收（3.60～3.66）。基本 STOP/CONT 交接、waitpid 一次性状态、信号致死状态、SA_NOCLDSTOP、进程组 wait 筛选及 SA_NOCLDWAIT/显式 SIG_IGN 自动回收通过（3.67～3.72）；完整 job control 与更广交错仍待补。 |
...
**2026-09-23 更新：部分修复，见 3.19、3.41、3.60～3.61、3.65～3.72。** handler 参数/返回值已有回归；默认 `users.signals` 已包含 SIGCHLD 基本断言、pipe/console 中断与部分传输、wait EINTR，`users.timers` 已包含 nanosleep EINTR。SA_RESTART 限定重启子集、CPU-bound IRQ 返回投递、基本 STOP/CONT 停止/恢复、SA_NOCLDSTOP、SA_NOCLDWAIT/显式 SIG_IGN 自动回收，以及 `waitpid` 的停止、继续、信号致死状态和进程组筛选已补；完整 job control 与更广阻塞/信号交错仍待实现和验收。
...
**验收：** handler signo/返回值、SIGCHLD 基本断言、CPU-bound 进程投递、STOP 后切出/CONT 后恢复、SA_NOCLDSTOP、SA_NOCLDWAIT/显式 SIG_IGN 自动回收、一次性 waitpid 停止/继续与信号致死状态、稳定成员关系的进程组 wait 筛选已有覆盖；还需完整进程组 job control、core dump 状态位、wait/console/sleep 更广的中断结果与交错。console 已有部分中断用例，不以此代替整组验收。
...
- [ ] D5：基本 SIGCHLD、pipe/console 中断、wait/nanosleep EINTR、CPU-bound IRQ 返回、STOP/CONT 交接、SA_NOCLDSTOP、SA_NOCLDWAIT/显式 SIG_IGN 自动回收、进程组 wait 筛选及 waitpid 停止/继续与信号致死状态报告已有用例；补完整 job control 与其余信号交错（020）。
