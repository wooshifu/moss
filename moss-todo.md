...
宿主回归直接编译生产连接方法，在初始化中同步移除服务；提交前源码会错误发布通道，新实现拒绝。九配置的真实内核 `containers.ipc_service_lifecycle` 还覆盖重名拒绝、按名连接、容量、断开后重连、注销及共享页回收；九构建配置、完整 CTest 43/43 与宿主相关回归 150/150 通过。x64 RelWithDebInfo 完整测试曾一次在独立的 `mm.concurrent/fault_unmap` 页引用断言处失败；该项随后定向连续 11 次及完整重跑通过，并发 VM 稳定性仍按 MOSS-008 跟踪。这只验收服务与连接的发布/移除边界，进程退出清理、在途消息、借用名称的外部有效期和全分配阶段失败回滚仍需专项验收。
