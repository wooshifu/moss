...
> 源码复核基线：2026-09-23，`604fe85`（当次未重跑 QEMU）。第 3.4～3.20 节保留此前实测及当时状态；后续实现与运行记录见第 3.21～3.76 节，源码与历史证据的对应关系见第 3.41 节。前一轮九预设运行见第 3.59～3.75 节。本文与 [todo.md](todo.md) 同步。
> ADR 迁移复核基线：2026-09-24，`c970c13`。第 3.75 节以前的测试数值不自动覆盖新服务路径或 IPC 收敛改动；九预设验收见 3.76，当前目标与进度另见 [todo.md 的 ADR 迁移清单](todo.md#adr-迁移)。
...
5. **测试已从基础语言检查升级为真实内核检查，覆盖仍有边界。** 3.75 的九预设 CTest 为 44/44，IPC 收敛后的 53/53 见 3.76；后者包含 Release 的三个 benchmark、ARM64 Debug console 登记探针，以及 ARM64/x64 的真实 IRQ/等待登记交错。默认用户套件包含 COW/OOM、恶意信号帧、部分用户复制故障、双 console 读者、管道与 nanosleep 信号中断、限定 SA_RESTART 重试、基本 STOP/CONT、SA_NOCLDSTOP、SA_NOCLDWAIT/显式 SIG_IGN 自动回收、信号 action 更新的受控交错及忽略动作的待处理清理、进程组 wait 筛选和成员变更唤醒、waitpid 停止/继续与信号致死状态报告及 1,000 次生命周期；这不证明全部 uaccess 故障、确定性调度竞争或真机可靠性。
...
| MOSS-006 | P0 | 容器与使用者的所有权 | 部分修复；伪 RCU 已替换为 LockedList/LockedHashMap，持有读者与双 CPU 交错有实测；启动设备已通过 DeviceManager/BootDriver 静态注册并激活；IRQ 注销等待回调（3.52），IPI 部分注册失败会回滚（3.53），旧 IPC 服务/通道发布与注销串行化（3.54）仅保留测试，生产不再链接该模块（3.76）；实际硬件回调、动态解绑及 native IPC 进程清理/在途消息仍待验收。 |
