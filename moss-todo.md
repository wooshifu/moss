...
> 状态复核：2026-09-06，提交基线：`b57422d`；后续工作区修复及实测见第 3.4～3.7 节。本文与 [todo.md](todo.md) 同步；未提交的实现不冒充已提交版本。
...
2. **部分基础修复已落地，但所有权验收未完成。** 链接脚本已预留 8 MiB 堆，扩容被限制在预留区内；PFA 已识别保留区并修正初始 buddy 对齐。工作区已验证 heap/PFA 对齐与释放、保留洞和多 bank 耗尽、当前布局及页表/元数据哨兵；RCU 自动退休仍可达节点、页引用并发及完整启动保留集合仍待完成。
...
原审计行号指 `e0e2bbc`，初次更新说明中的符号和路径按 `b57422d` 核对；工作区修复另标。请用“路径 + 符号 + 不变量”定位。每项完整验收仍是关闭要求；只有第 3.3～3.7 节明确列出的覆盖可以算已执行，不能把新增普通测试外推成所有审计验收通过。`[x]` 只关闭该行限定的子任务，`[ ]` 包括待实现、部分实现及待专项验收。
...
| MOSS-004 | P0 | 堆与页表/PFA 所有权重叠 | 当前布局、活动页表树/早期表池及元数据哨兵、耗尽、坏布局启动拒绝专项验收通过，待修复提交，见 3.7。 |
| MOSS-005 | P0 | 启动保留区未排除 | 多 bank、保留洞、非对齐/重叠、容量/溢出及耗尽已验证；低地址启动区仍保守保留，PVH 异常表/完整保留集合待补，见 3.5～3.6。 |
| MOSS-006 | P0 | RCU 退休和宽限期 | 未修复；`RcuList::push_front` / `RcuPtr::store_rcu` / callback drain 契约仍在。 |
...
| MOSS-013 | P1 | 对齐 / buddy / 释放契约 | heap/PFA 对齐、释放归属、保留洞分段/耗尽专项验收已通过，待修复提交；页引用并发不在此结论内，见 3.5。 |
...
| MOSS-029 | P2 | 计时源和 ISA 能力 | 部分实现；x86 双频率校准、RV64 SBI TIME 已落地，异常能力与时钟误差验收待补。 |
...
当前没有将任何包含未完成验收的整项标为关闭；已完成的有限子任务在第 11 节单独勾选。以下保留原始问题细节，避免修复后丢失回归依据。
...
**2026-09-06 更新：专项验收通过，待修复提交。** `PageFrameAllocator::initialize_free_lists` 循环选择符合物理地址对齐、剩余长度和保留区约束的 order。工作区 heap 修复及五项用例见 3.4；PFA 原分配头/order、所有页的引用/标志、完整范围检查及两项真实套件见 3.5。六配置 CTest 与 ARM64/RV64 固件边界检查已通过本项所列对齐、错误释放和保留洞后的分段/耗尽验收。此结论不覆盖页引用 API 自身的并发/下溢或完整 COW 生命周期；那些仍需 008～010/028 验收。
