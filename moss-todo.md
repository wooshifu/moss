...
| MOSS-002 | P0 | 用户域 / uaccess / 调试旁路 | 部分修复（3.13、3.20～3.21、3.31～3.39、3.42～3.45）；共享 uaccess/COW OOM、拥有型地址空间、软件 VM 事务、同步页租约、三架构 TLB 协议、双发布者交错、活动 root 拥有权/本核退休、首次 CPU 注册两顺序、单次 exec 输入版本绑定、缺页/unmap 与缺页/fork 交错及多线程 exec 失败关闭已补；完整共享 exec、异步页 pin 与完整并发验收未完成。 |
